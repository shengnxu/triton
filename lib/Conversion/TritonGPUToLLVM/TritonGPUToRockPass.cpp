...
struct DotOpRewritePattern : public OpRewritePattern<triton::DotOp> {
  ...
  matchAndRewrite(triton::DotOp op,
    ...
    if (!matADefiningOp || !matBDefiningOp) {
      ...
      dotInLoop = true;
      return matchAndRewriteDotInLoop(op, rewriter);
    }
...
private:
  ...
  matchAndRewriteDotInLoop(triton::DotOp op,
    ...
    if (dotOpA && dotOpB) {
      ...
      llvm::outs() << "matA is block argument " << dotOpAIdx << "\n";
      llvm::outs() << "matB is block argument " << dotOpBIdx << "\n";
      llvm::outs() << "matC is block argument " << accIdx << "\n";
    } else
      return emitError(loc) << "Invalid block arguments!\n";
    ...
    for (auto it = loopBB->pred_begin(), e = loopBB->pred_end(); it != e;
      ...
      llvm::outs() << "Begin: br has " << brOp->getNumOperands()
                   << " operands\n";
      ...
      if (!extractSliceOpA || !extractSliceOpB)
        return emitError(loc) << "Failed to find the extract_slice op.\n";
      // Save their type since we need to add them into the block argument list
      // of loopBB
      ...
      if (!insertSliceOpA || !insertSliceOpB)
        return emitError(loc) << "Failed to find insert_slice op.\n";
      ...
      if (isEntryBB) {
        ...
        auto _tid = rewriter.create<WorkitemIdOp>(loc, rewriter.getIndexType());
        ...
        llvm::outs() << "entry br has " << brOp->getNumOperands()
                     << " operands\n";
      }
      ...
      rewriter.eraseOp(cvtOpA);
      rewriter.eraseOp(cvtOpB);
      brOp->eraseOperands(dotOpAIdx, 2);
      brOp->eraseOperand(accIdx);
      llvm::outs() << "Now br has " << brOp->getNumOperands() << " operands\n";
      ...
      llvm::outs() << "End br has " << brOp->getNumOperands() << " operands\n";
      // if (isEntryBB) {
      //   llvm::outs() << "Entry DUMP:\n";
      //   predBB->dump();
      // }
    }
    ...
    printVec(LDSTensorAShape);
    printVec(LDSTensorBShape);
    int64_t ldsBlockASize = product<long int>(LDSTensorAShape);
    int64_t ldsBlockBSize = product<long int>(LDSTensorBShape);
    llvm::outs() << "ldsBlockASize: " << ldsBlockASize << "\n";
    llvm::outs() << "ldsBlockBSize: " << ldsBlockBSize << "\n";
    auto ldsAMemRefType = MemRefType::get(
        {ldsBlockASize}, elementType, AffineMap{}, workgroupMemoryAddressSpace);
    auto ldsBMemRefType = MemRefType::get(
        {ldsBlockBSize}, elementType, AffineMap{}, workgroupMemoryAddressSpace);
    ...
    // auto currentBB = op->getBlock();
    // llvm::outs() << "Current Block DUMP:\n";
    // currentBB->dump();
    op.erase();
    ...
    if (!dotOpC.hasOneUse())
      return emitError(loc) << "dotOpC has more than one users!\n";
    ...
    //  Dump
    // llvm::outs() << "Loop head block DUMP:\n";
    // loopBB->dump();
