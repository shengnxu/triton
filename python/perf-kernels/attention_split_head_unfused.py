...
def _fwd_kernel_qk(
    ...
    sm_scale,
    QK,
    stride_qz, stride_qm, stride_qg, stride_qh, stride_qk,
    stride_kz, stride_kn, stride_kg, stride_kh, stride_kk,
    stride_qkz, stride_qkm, stride_qkg, stride_qkh, stride_qkn,
    Z,
    N_CTX_Q,
    N_CTX_K,
    H: tl.constexpr,
    ...
    QK_base = QK + off_h * stride_qkh + off_z * stride_qkz + off_g * stride_qkg
    QK_block_ptr = tl.make_block_ptr(
        base=QK_base,
        shape=(N_CTX_Q, kv_len),
        strides=(stride_qkm, stride_qkn),
        offsets=(0, 0),
    ...
    for start_n in range(0, kv_len, BLOCK_N):
        ...
        # update pointers
        tl.store(QK_block_ptr, qk)
        K_block_ptr = tl.advance(K_block_ptr, (0, BLOCK_N))
        QK_block_ptr = tl.advance(QK_block_ptr, (0, BLOCK_N))
...
def _fwd_kernel_pv(
    ...
    Out,
    lse,
    stride_qkz, stride_qkm, stride_qkg, stride_qkh, stride_qkk,
    stride_vz, stride_vk, stride_vg, stride_vh, stride_vn,
    stride_oz, stride_om, stride_og, stride_oh, stride_on,
    Z,
    N_CTX_Q,
    N_CTX_K,
    H: tl.constexpr,
    ...
    QK_block_ptr = tl.make_block_ptr(
        base=QK + off_h * stride_qkh + off_z * stride_qkz + off_g * stride_qkg,
        shape=(N_CTX_Q, kv_len),
        strides=(stride_qkm, stride_qkk),
        offsets=(0, 0),
        block_shape=(BLOCK_M, BLOCK_N),
        order=(1, 0),
    )
...
class _attention(torch.autograd.Function):
    ...
    def forward(cls, q, k, v, scale_float):
        ...
        qk = torch.empty((B, M, G, H, Kkv), device=q.device, dtype=torch.float32)
        ...
        _fwd_kernel_qk[grid](
            ...
            **_strides(qk, "qkz", "qkm", "qkg", "qkh", "qkn"),
            Z=B,
            H=H,
            G=G,
            N_CTX_Q=M,
        ...
        _fwd_kernel_pv[grid](
            ...
            Out=o,
            lse=lse,
            **_strides(qk, "qkz", "qkm", "qkg", "qkh", "qkk"),
            **_strides(v, "vz", "vk", "vg", "vh", "vn"),
