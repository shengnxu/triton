...
def benchmark_FA_fwd(BATCH, H, N_CTX, D_HEAD, rep, dtype=torch.float16, device="cuda"):
    q = torch.randn((BATCH, H, N_CTX, D_HEAD), dtype=dtype, device="cuda", requires_grad=True)
    k = torch.randn((BATCH, H, N_CTX, D_HEAD), dtype=dtype, device="cuda", requires_grad=True)
    v = torch.randn((BATCH, H, D_HEAD, N_CTX), dtype=dtype, device="cuda", requires_grad=True)
    sm_scale = 1.3
...
def main(args=None):
    ...
    parser.add_argument("-nheads", type=int, default=48)
    parser.add_argument("-d", type=int, default=128)
    parser.add_argument("-seqlen", type=int, default=16384)
    parser.add_argument("-rep", type=int, default=100)
    parser.add_argument("-do_bench", action='store_true', default=False, help='use do_bench()')
    ...
    rep = parsed_args.rep
    use_do_bench = parsed_args.do_bench

    if use_do_bench:
        bench_flash_attention.run(save_path='.', print_data=True)
    else:
        benchmark_FA_fwd(bs, nheads, seqlen, d, rep)
