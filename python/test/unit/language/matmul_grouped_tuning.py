...
def test_gemm(SIZE_M, SIZE_N, SIZE_K, num_warps, BLOCK_SIZE_M, BLOCK_SIZE_N, BLOCK_SIZE_K, SPLIT_K, kpack, mPerWave, GROUP_SIZE_M):
    a = torch.randn((SIZE_M, SIZE_K), device='cuda', dtype=torch.float16)
    b = torch.randn((SIZE_K, SIZE_N), device='cuda', dtype=torch.float16)
    c = torch.zeros((SIZE_M, SIZE_N), device=a.device, dtype=torch.float32)
    # grid = lambda META: (1, )
    grid = lambda META: (
        triton.cdiv(SIZE_M, int(BLOCK_SIZE_M)) * triton.cdiv(SIZE_N, int(BLOCK_SIZE_N)),
        int(SPLIT_K)
    )

    tuningSpace = open('tuningSpace_768_1408_4608.txt', 'r')
    params = tuningSpace.readlines()
    print("reading done")

    for param in params:
        splitParam = param.split(sep=',')
        BLOCK_SIZE_M = splitParam[0]
        BLOCK_SIZE_N = splitParam[1]
        BLOCK_SIZE_K = splitParam[2]
        SPLIT_K = splitParam[3]
        GROUP_SIZE_M = splitParam[4]
        mPerWave = splitParam[5]
        kpack = splitParam[6]
        num_warps = splitParam[7]
        print(param.strip())
        matmul_kernel[grid](a_ptr=a, b_ptr=b, c_ptr=c,
                        ...
                        kpack=int(kpack), mPerWave=int(mPerWave)
                        )
...
def main(args=None):
    ...
    GROUP_M = parsed_args.groupM
    kpack = parsed_args.kpack
    mPerWave = parsed_args.mPerWave

    
    test_gemm(M, N, K, num_warps, BLOCK_M, BLOCK_N, BLOCK_K, SPLIT_K, kpack, mPerWave, GROUP_M)
