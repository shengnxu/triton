...
def test_compile_link_matmul_no_specialization():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        kernel_path = write_triton_kernels("/root/triton/python/test/unit/tools", kernel_src, kernel_utils_src)
        compile_aot_kernel_no_specialization("/root/triton/python/test/unit/tools", kernel_path, dtype, BM, BN, BK)
        link_aot_kernels("/root/triton/python/test/unit/tools")

        # compile test case
        M, N, K = 16, 16, 16
        #gen_kernel_library(tmp_dir, "libkernel.so")
        #gen_test_bin(tmp_dir, M, N, K)
        gen_kernel_library("/root/triton/python/test/unit/tools", "libkernel.so")
        gen_test_bin("/root/triton/python/test/unit/tools", M, N, K)

        # initialize test data
        #a, b, a_path, b_path, c_path = generate_matmul_test_data(tmp_dir, M, N, K)
        a, b, a_path, b_path, c_path = generate_matmul_test_data("/root/triton/python/test/unit/tools", M, N, K)

        # run test case
        env = os.environ.copy()
        #env["LD_LIBRARY_PATH"] = tmp_dir
        #subprocess.run(["./test", a_path, b_path, c_path], env=env, check=True, cwd=tmp_dir)
        env["LD_LIBRARY_PATH"] = "/root/triton/python/test/unit/tools" 
        subprocess.run(["/root/triton/python/test/unit/tools/test", a_path, b_path, c_path], env=env, check=True, cwd=tmp_dir)
...
def test_compile_link_matmul():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        #link_aot_kernels(tmp_dir)
        kernel_path = write_triton_kernels("/root/triton/python/test/unit/tools", kernel_src, kernel_utils_src)
        compile_aot_kernels("/root/triton/python/test/unit/tools", kernel_path, dtype, BM, BN, BK, ha_hb_hints=["", ":16"])
        link_aot_kernels("/root/triton/python/test/unit/tools")
