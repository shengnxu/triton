...
def make_hash(fn, **kwargs):
    if isinstance(fn, triton.runtime.JITFunction):
        ...
        key = f"{fn.cache_key}-{''.join(signature.values())}-{configs_key}-{constants}-{num_warps}-{kpack}-{mPerWave}-{num_stages}-{debug}"
...
def compile(fn, **kwargs):
    ...
    stages["ttgir"] = (lambda path: parse_mlir_module(path, context),
                       lambda src: optimize_ttgir(ttir_to_ttgir(src, num_warps, kpack, mPerWave), num_stages, arch))
