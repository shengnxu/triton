...
bool isLoadFromTensorPtr(triton_rocm::LoadOp op);

/// Returns true if the store is for TMA
bool isStoreToTensorPtr(triton_rocm::StoreOp op);
...
triton_rocm::gpu_rocm::SharedEncodingAttr getSharedEncoding(RankedTensorType tensorTy);
...
public:
  ...
  std::string dump(triton_rocm::FuncOp func) const;
  void dumpToFile(triton_rocm::FuncOp func, const std::string &filename) const;
