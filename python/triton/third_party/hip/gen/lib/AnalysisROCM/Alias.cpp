...
void SharedMemoryAliasAnalysis::visitOperation(
  ...
  if (maybeSharedAllocationOp(op)) {
    ...
    if (isa<triton_rocm::gpu_rocm::ExtractSliceOp, triton_rocm::TransOp,
            triton_rocm::nvidia_gpu::ExtractMBarrierOp>(op)) {
      // extract_slice %src
      // trans %src
      aliasInfo = AliasInfo(operands[0]->getValue());
      pessimistic = false;
    } else if (isa<tensor::InsertSliceOp, triton_rocm::gpu_rocm::InsertSliceAsyncOp,
                   triton_rocm::nvidia_gpu::InsertSliceAsyncV2Op>(op)) {
      // insert_slice_async %src, %dst, %index
      // insert_slice %src into %dst[%offsets]
      aliasInfo = AliasInfo(operands[1]->getValue());
      pessimistic = false;
    } else if (isa<triton_rocm::nvidia_gpu::StoreAsyncOp>(op)) {
      aliasInfo = AliasInfo(operands[0]->getValue());
      pessimistic = false;
    } else if (triton_rocm::gpu_rocm::isSharedEncoding(result)) {
