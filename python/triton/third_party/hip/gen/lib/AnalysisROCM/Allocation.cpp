...
using ::mlir::triton_rocm::gpu_rocm::getUniqueContigPerThread;
...
SmallVector<unsigned> getRepShapeForCvtLayout(triton_rocm::gpu_rocm::ConvertLayoutOp op) {
...
getScratchConfigForCvtLayout(triton_rocm::gpu_rocm::ConvertLayoutOp op, unsigned &inVec,
                             unsigned &outVec) {
...
SmallVector<unsigned>
getScratchConfigForStoreAsync(triton_rocm::nvidia_gpu::StoreAsyncOp op) {
  auto srcTy = op.getSrc().getType().cast<RankedTensorType>();
  return convertType<unsigned, int64_t>(getShapePerCTA(srcTy));
}

// TODO: extend beyond scalars
SmallVector<unsigned> getScratchConfigForAtomicRMW(triton_rocm::AtomicRMWOp op) {
...
SmallVector<unsigned> getScratchConfigForAtomicCAS(triton_rocm::AtomicCASOp op) {
...
private:
  ...
  void getExplicitValueSize(Operation *op) {
    ...
    for (Value result : op->getResults()) {
      if (triton_rocm::gpu_rocm::isSharedEncoding(result)) {
        // Bytes could be a different value once we support padding or other
        // allocation policies.
        auto tensorType = result.getType().dyn_cast<RankedTensorType>();
        auto shapePerCTA = triton_rocm::gpu_rocm::getShapePerCTA(tensorType);
    ...
    if (isa<triton_rocm::nvidia_gpu::AllocMBarrierOp>(op)) {
  ...
  void getScratchValueSize(Operation *op) {
    ...
    if (auto reduceOp = dyn_cast<triton_rocm::ReduceOp>(op)) {
      ReduceOpHelper helper(reduceOp);
      unsigned bytes = helper.getScratchSizeInBytes();
      maybeAddScratchBuffer<BufferT::BufferKind::Scratch>(op, bytes,
                                                          scratchAlignment);
    } else if (auto scanOp = dyn_cast<triton_rocm::ScanOp>(op)) {
      ScanLoweringHelper helper(scanOp);
      unsigned bytes = helper.getScratchSizeInBytes();
      maybeAddScratchBuffer<BufferT::BufferKind::Scratch>(op, bytes,
                                                          scratchAlignment);
    } else if (auto cvtLayout = dyn_cast<triton_rocm::gpu_rocm::ConvertLayoutOp>(op)) {
      ...
      unsigned elems = std::accumulate(smemShape.begin(), smemShape.end(), 1,
                                       std::multiplies{});
      auto bytes =
          srcTy.getElementType().isa<triton_rocm::PointerType>()
              ? elems * kPtrBitWidth / 8
              : elems * std::max<int>(8, srcTy.getElementTypeBitWidth()) / 8;
      maybeAddScratchBuffer<BufferT::BufferKind::Scratch>(op, bytes,
                                                          scratchAlignment);
    } else if (auto storeAsyncOp =
                   dyn_cast<triton_rocm::nvidia_gpu::StoreAsyncOp>(op)) {
    ...
    } else if (auto atomicRMWOp = dyn_cast<triton_rocm::AtomicRMWOp>(op)) {
      ...
      if (value.getType().dyn_cast<RankedTensorType>()) {
        ...
        unsigned elems = std::accumulate(smemShape.begin(), smemShape.end(), 1,
                                         std::multiplies{});
        auto elemTy =
            value.getType().cast<triton_rocm::PointerType>().getPointeeType();
        auto bytes =
            elemTy.isa<triton_rocm::PointerType>()
                ? elems * kPtrBitWidth / 8
                : elems * std::max<int>(8, elemTy.getIntOrFloatBitWidth()) / 8;
        maybeAddScratchBuffer<BufferT::BufferKind::Scratch>(op, bytes,
                                                            scratchAlignment);
      }
    } else if (auto atomicCASOp = dyn_cast<triton_rocm::AtomicCASOp>(op)) {
      // only scalar requires scratch memory
      // make it explicit for readability
      auto value = op->getOperand(0);
      if (value.getType().dyn_cast<RankedTensorType>()) {
        ...
        auto elemTy =
            value.getType().cast<triton_rocm::PointerType>().getPointeeType();
        auto bytes = elemTy.isa<triton_rocm::PointerType>()
                         ? elems * kPtrBitWidth / 8
  ...
  void resolveLiveness() {
    ...
    auto getValueLivenessRange = [&](Value value) {
      ...
      if (value.getDefiningOp() &&
          isa<triton_rocm::nvidia_gpu::AllocMBarrierOp>(value.getDefiningOp())) {
...
void Allocation::run(FuncAllocMapT &funcAllocMap) {
  triton_rocm::AllocationAnalysis(getOperation(), &funcAllocMap, this);
}
