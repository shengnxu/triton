...
class MakeRangeOpAxisInfoVisitor final
    : public AxisInfoVisitorImpl<triton_rocm::MakeRangeOp> {
public:
  using AxisInfoVisitorImpl<triton_rocm::MakeRangeOp>::AxisInfoVisitorImpl;

  AxisInfo
  getAxisInfo(triton_rocm::MakeRangeOp op,
              ArrayRef<const dataflow::Lattice<AxisInfo> *> operands) override {
...
private:
  ...
  int64_t getDivisibility(OpTy op, const AxisInfo &lhs, const AxisInfo &rhs,
    ...
    // gcd(d_lhs, d_rhs)
    auto elemSize = 1;
    if constexpr (std::is_same_v<OpTy, triton_rocm::AddPtrOp>) {
      //  %ptr = addptr %lhs, %rhs
      // is equivalent to
      //  %0 = mul %lhs, %elemSize
      //  %ptr = add %0, %rhs
      elemSize = std::max<unsigned int>(
          1, triton_rocm::getPointeeBitWidth(op.getPtr().getType()) / 8);
    }
  ...
  std::optional<int64_t> getConstantValue(OpTy op, const AxisInfo &lhs,
    ...
    if (lhs.getConstantValue().has_value() &&
      ...
      if constexpr (std::is_same_v<OpTy, arith::AddIOp> ||
                    std::is_same_v<OpTy, triton_rocm::AddPtrOp> ||
...
class SplatOpAxisInfoVisitor final
    : public AxisInfoVisitorImpl<triton_rocm::SplatOp> {
public:
  using AxisInfoVisitorImpl<triton_rocm::SplatOp>::AxisInfoVisitorImpl;

  AxisInfo
  getAxisInfo(triton_rocm::SplatOp op,
              ArrayRef<const dataflow::Lattice<AxisInfo> *> operands) override {
...
class LoadOpAxisInfoVisitor final : public AxisInfoVisitorImpl<triton_rocm::LoadOp> {
public:
  using AxisInfoVisitorImpl<triton_rocm::LoadOp>::AxisInfoVisitorImpl;

  AxisInfo
  getAxisInfo(triton_rocm::LoadOp op,
              ArrayRef<const dataflow::Lattice<AxisInfo> *> operands) override {
...
class ExpandDimsOpAxisInfoVisitor final
    : public AxisInfoVisitorImpl<triton_rocm::ExpandDimsOp> {
public:
  using AxisInfoVisitorImpl<triton_rocm::ExpandDimsOp>::AxisInfoVisitorImpl;

  AxisInfo
  getAxisInfo(triton_rocm::ExpandDimsOp op,
              ArrayRef<const dataflow::Lattice<AxisInfo> *> operands) override {
    AxisInfo opInfo = operands[0]->getValue();
...
class BroadcastOpAxisInfoVisitor final
    : public AxisInfoVisitorImpl<triton_rocm::BroadcastOp> {
public:
  using AxisInfoVisitorImpl<triton_rocm::BroadcastOp>::AxisInfoVisitorImpl;

  AxisInfo
  getAxisInfo(triton_rocm::BroadcastOp op,
              ArrayRef<const dataflow::Lattice<AxisInfo> *> operands) override {
    Type _retTy = *op->result_type_begin();
...
AxisInfoAnalysis::AxisInfoAnalysis(DataFlowSolver &solver)
  ...
  visitors.append<CastOpAxisInfoVisitor<arith::ExtSIOp>,
                  ...
                  CastOpAxisInfoVisitor<triton_rocm::PtrToIntOp>,
                  CastOpAxisInfoVisitor<triton_rocm::IntToPtrOp>,
                  CastOpAxisInfoVisitor<triton_rocm::gpu_rocm::ConvertLayoutOp>,
                  CastOpAxisInfoVisitor<mlir::UnrealizedConversionCastOp>,
                  CastOpAxisInfoVisitor<triton_rocm::BitcastOp>>();
  ...
  visitors.append<AddSubOpAxisInfoVisitor<triton_rocm::AddPtrOp>,
...
unsigned ModuleAxisInfoAnalysis::getPtrContiguity(Value ptr) {
  ...
  auto uniqueContigPerThread =
      triton_rocm::gpu_rocm::getUniqueContigPerThread(layout, tensorTy.getShape());
...
unsigned ModuleAxisInfoAnalysis::getPtrAlignment(Value ptr) {
  ...
  auto elemNumBits = triton_rocm::getPointeeBitWidth(tensorTy);
...
unsigned ModuleAxisInfoAnalysis::getMaskAlignment(Value mask) {
  ...
  if (!axisInfo)
    return 1;
  auto maskOrder = triton_rocm::gpu_rocm::getOrder(tensorTy.getEncoding());
