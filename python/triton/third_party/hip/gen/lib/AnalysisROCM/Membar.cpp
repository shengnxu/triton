...
void MembarAnalysis::resolve(FunctionOpInterface funcOp,
  ...
  funcOp.walk<WalkOrder::PreOrder>([&](Block *block) {
    block->walk([&](triton_rocm::ReturnOp returnOp) {
...
void MembarAnalysis::visitTerminator(Operation *op,
  ...
  if (isa<triton_rocm::ReduceReturnOp>(op) || isa<triton_rocm::ScanReturnOp>(op) ||
...
void MembarAnalysis::update(Operation *op, BlockInfo *blockInfo,
                            FuncBlockInfoMapT *funcBlockInfoMap,
                            OpBuilder *builder) {
  if (isa<triton_rocm::gpu_rocm::ExtractSliceOp>(op) ||
      isa<triton_rocm::gpu_rocm::AllocTensorOp>(op) || isa<triton_rocm::TransOp>(op)) {
  ...
  if (isa<triton_rocm::gpu_rocm::AsyncWaitOp, triton_rocm::gpu_rocm::AsyncBulkWaitOp>(op) &&
    ...
    for (Value value : op->getOperands()) {
      for (auto bufferId : allocation->getBufferIds(value)) {
        if (bufferId != Allocation::InvalidBufferId) {
          if (isa<triton_rocm::gpu_rocm::InsertSliceAsyncOp>(op) ||
