...
int getParentAxis(Attribute layout, int axis) {
  if (auto sliceEncoding = layout.dyn_cast<triton_rocm::gpu_rocm::SliceEncodingAttr>()) {
...
SmallVector<unsigned> getParentOrder(Attribute layout) {
  ...
  return triton_rocm::gpu_rocm::getOrder(layout);
}
...
SmallVector<unsigned> ReduceOpHelper::getOrderWithAxisAtBeginning() {
  ...
  auto order = triton_rocm::gpu_rocm::getOrder(srcLayout);
...
unsigned ReduceOpHelper::getThreadOffsetOnReductionAxis() {
  ...
  if (auto sliceLayout =
          srcLayout.dyn_cast<mlir::triton_rocm::gpu_rocm::SliceEncodingAttr>()) {
    auto parentLayout = sliceLayout.getParent();
    auto threadsPerWarp = triton_rocm::gpu_rocm::getThreadsPerWarp(parentLayout);
    threadOffset = threadsPerWarp[sliceLayout.getDim()];
  } else {
    auto threadsPerWarp = triton_rocm::gpu_rocm::getThreadsPerWarp(srcLayout);
    auto order = triton_rocm::gpu_rocm::getOrder(srcLayout);
...
bool shouldUseDistSmem(Attribute srcLayout, Attribute dstLayout) {
  unsigned numCTAs = triton_rocm::gpu_rocm::getNumCTAs(srcLayout);
  assert(numCTAs == triton_rocm::gpu_rocm::getNumCTAs(dstLayout) &&
  ...
  // implemented yet
  if (auto sliceLayout = srcLayout.dyn_cast<triton_rocm::gpu_rocm::SliceEncodingAttr>()) {
    auto dim = sliceLayout.getDim();
    auto CTAsPerCGA = triton_rocm::gpu_rocm::getCTAsPerCGA(sliceLayout.getParent());
    if (CTAsPerCGA[dim] != 1)
  ...
  if (auto sliceLayout = dstLayout.dyn_cast<triton_rocm::gpu_rocm::SliceEncodingAttr>()) {
    auto dim = sliceLayout.getDim();
    auto CTAsPerCGA = triton_rocm::gpu_rocm::getCTAsPerCGA(sliceLayout.getParent());
    if (CTAsPerCGA[dim] != 1)
      return true;
  ...
  auto srcCTALayout = triton_rocm::gpu_rocm::getCTALayout(srcLayout);
  auto dstCTALayout = triton_rocm::gpu_rocm::getCTALayout(dstLayout);
...
unsigned ReduceOpHelper::getInterWarpSize() {
  ...
  return std::min(srcReduceDimSize / sizeIntraWarps,
                  triton_rocm::gpu_rocm::getWarpsPerCTA(getSrcLayout())[axis]);
}

unsigned ReduceOpHelper::getIntraWarpSize() {
  auto srcReduceDimSize = static_cast<unsigned>(srcShape[axis]);
  return std::min(srcReduceDimSize,
                  triton_rocm::gpu_rocm::getThreadsPerWarp(getSrcLayout())[axis]);
}
...
unsigned ReduceOpHelper::getInterWarpSizeWithUniqueData() {
  ...
  return std::min(srcReduceDimSize / sizeIntraWarps,
                  triton_rocm::gpu_rocm::getWarpsPerCTAWithUniqueData(
                      getSrcLayout(), getSrcShape())[axis]);
}

unsigned ReduceOpHelper::getIntraWarpSizeWithUniqueData() {
  auto srcReduceDimSize = static_cast<unsigned>(srcShape[axis]);
  unsigned elementPerThreads = triton_rocm::gpu_rocm::getUniqueContigPerThread(
      getSrcLayout(), getSrcShape())[axis];
  return std::min(srcReduceDimSize / elementPerThreads,
                  triton_rocm::gpu_rocm::getThreadsPerWarpWithUniqueData(
                      getSrcLayout(), getSrcShape())[axis]);
}
...
unsigned ReduceOpHelper::getThreadsReductionAxis() {
  ...
  return triton_rocm::gpu_rocm::getThreadsPerWarpWithUniqueData(srcLayout,
                                                      srcShape)[axis] *
         triton_rocm::gpu_rocm::getWarpsPerCTAWithUniqueData(srcLayout, srcShape)[axis];
}

bool ReduceOpHelper::isWarpSynchronous() {
  auto srcLayout = getSrcLayout();
  auto srcShape = getSrcShape();
  return triton_rocm::gpu_rocm::getWarpsPerCTAWithUniqueData(srcLayout, srcShape)[axis] ==
         1;
}
...
bool ReduceOpHelper::isReduceWithinCTA() {
  auto axis = getAxis();
  auto srcLayout = getSrcLayout();
  auto CTASplitNum = mlir::triton_rocm::gpu_rocm::getCTASplitNum(srcLayout);
...
bool ReduceOpHelper::isSupportedLayout() {
  ...
  auto srcLayout = getSrcLayout();
  if (srcLayout.isa<triton_rocm::gpu_rocm::BlockedEncodingAttr>()) {
    return true;
  }
  if (auto mmaLayout = srcLayout.dyn_cast<triton_rocm::gpu_rocm::MmaEncodingAttr>()) {
    if (mmaLayout.isAmpere() || mmaLayout.isHopper()) {
      return true;
    }
  }
  if (auto mfmaLayout = srcLayout.dyn_cast<triton_rocm::gpu_rocm::MfmaEncodingAttr>()) {
    return true;
  }
  if (auto sliceLayout = srcLayout.dyn_cast<triton_rocm::gpu_rocm::SliceEncodingAttr>()) {
    return true;
...
unsigned ScanLoweringHelper::getNonAxisNumElementsPerThread() {
  SmallVector<unsigned> sizePerThreads =
      triton_rocm::gpu_rocm::getContigPerThread(getEncoding());
...
unsigned ScanLoweringHelper::getAxisNumThreadsPerWarp() {
  return triton_rocm::gpu_rocm::getThreadsPerWarp(getEncoding())[getAxis()];
}

unsigned ScanLoweringHelper::getAxisNumThreadsPerWarpWithUniqueData() {
  return triton_rocm::gpu_rocm::getThreadsPerWarpWithUniqueData(getEncoding(),
                                                      getShape())[getAxis()];
}

unsigned ScanLoweringHelper::getNonAxisNumThreadsPerWarp() {
  auto threadsPerWarp = triton_rocm::gpu_rocm::getThreadsPerWarp(getEncoding());
...
unsigned ScanLoweringHelper::getNonAxisNumThreadsPerCTA() {
  ...
  auto warpsPerCTA = triton_rocm::gpu_rocm::getWarpsPerCTA(getEncoding());
...
unsigned ScanLoweringHelper::getAxisNumWarps() {
  return triton_rocm::gpu_rocm::getWarpsPerCTA(getEncoding())[getAxis()];
}

unsigned ScanLoweringHelper::getAxisNumWarpsWithUniqueData() {
  return triton_rocm::gpu_rocm::getWarpsPerCTAWithUniqueData(getEncoding(),
                                                   getShape())[getAxis()];
}

unsigned ScanLoweringHelper::getAxisNumBlocks() {
  auto sizePerThreads = triton_rocm::gpu_rocm::getSizePerThread(getEncoding());
  auto threadsPerWarp = triton_rocm::gpu_rocm::getThreadsPerWarp(getEncoding());
  auto warpsPerCTA = triton_rocm::gpu_rocm::getWarpsPerCTA(getEncoding());
...
bool ScanLoweringHelper::isSupported() {
  ...
  if (!isa<triton_rocm::gpu_rocm::BlockedEncodingAttr>(getEncoding()))
    return false;
...
unsigned ScanLoweringHelper::getScratchSizeInBytes() {
  ...
  unsigned numWarps = triton_rocm::gpu_rocm::TritonGPUROCMDialect::getNumWarps(mod);
...
triton_rocm::gpu_rocm::BlockedEncodingAttr ScanLoweringHelper::getEncoding() {
  return srcEncoding.cast<triton_rocm::gpu_rocm::BlockedEncodingAttr>();
}
...
unsigned ScanLoweringHelper::getAxisElementStride() {
  auto order = triton_rocm::gpu_rocm::getOrder(getEncoding());
  unsigned stride = 1;
...
bool maybeSharedAllocationOp(Operation *op) {
  ...
  return dialect &&
         (dialect->getTypeID() ==
              mlir::TypeID::get<triton_rocm::gpu_rocm::TritonGPUROCMDialect>() ||
          dialect->getTypeID() ==
              mlir::TypeID::get<triton_rocm::nvidia_gpu::TritonNvidiaGPUROCMDialect>() ||
          dialect->getTypeID() == mlir::TypeID::get<triton_rocm::TritonROCMDialect>() ||
          dialect->getTypeID() == mlir::TypeID::get<arith::ArithDialect>() ||
          dialect->getTypeID() == mlir::TypeID::get<tensor::TensorDialect>());
}

bool maybeAliasOp(Operation *op) {
  return isa<triton_rocm::gpu_rocm::ExtractSliceOp>(op) || isa<triton_rocm::TransOp>(op) ||
         isa<triton_rocm::gpu_rocm::InsertSliceAsyncOp>(op) ||
         isa<triton_rocm::nvidia_gpu::InsertSliceAsyncV2Op>(op) ||
         isa<triton_rocm::nvidia_gpu::StoreAsyncOp>(op) ||
         isa<tensor::InsertSliceOp>(op);
}

bool supportMMA(triton_rocm::DotOp op, int version) {
  ...
  if (version == 3) {
    if (::triton_rocm::tools::getBoolEnv("DISABLE_MMA_V3"))
      return false;
    auto retType = op.getResult().getType().cast<RankedTensorType>();
    auto retShapePerCTA = triton_rocm::gpu_rocm::getShapePerCTA(retType);
...
bool supportMFMA(triton_rocm::DotOp op) {
...
bool supportWMMA(triton_rocm::DotOp op) {
  auto aTy = op.getA().getType().cast<RankedTensorType>();
  auto bTy = op.getB().getType().cast<RankedTensorType>();
...
static bool isMmaToMmaShortcut(Attribute srcEncoding, Attribute dstEncoding) {
  auto src = srcEncoding.dyn_cast<triton_rocm::gpu_rocm::MmaEncodingAttr>();
  auto dst = dstEncoding.dyn_cast<triton_rocm::gpu_rocm::MmaEncodingAttr>();
  if (!src || !dst)
    return false;
...
bool matchMmaV3AndDotOperandLayout(RankedTensorType srcTy,
  ...
  auto mmaLayout = srcLayout.cast<triton_rocm::gpu_rocm::MmaEncodingAttr>();
  auto dotOperandLayout = dstLayout.cast<triton_rocm::gpu_rocm::DotOperandEncodingAttr>();
...
bool isMfmaToDotShortcut(RankedTensorType &srcTy, RankedTensorType &dstTy) {
  auto srcLayout = srcTy.getEncoding();
  auto dstLayout = dstTy.getEncoding();
  auto mfmaLayout = srcLayout.cast<triton_rocm::gpu_rocm::MfmaEncodingAttr>();
  auto dotOperandLayout = dstLayout.cast<triton_rocm::gpu_rocm::DotOperandEncodingAttr>();
...
static triton_rocm::MakeTensorPtrOp getMakeTensorPtrOpImpl(Operation *op, Value v) {

  if (auto makeTensorPtrOp = dyn_cast<triton_rocm::MakeTensorPtrOp>(op)) {
    return makeTensorPtrOp;
  }

  if (auto advanceOp = dyn_cast<triton_rocm::AdvanceOp>(op)) {
...
triton_rocm::MakeTensorPtrOp getMakeTensorPtrOp(Value v) {
  ...
  if (Operation *definingOp = v.getDefiningOp()) {
    ...
    } else if (auto funcOp = dyn_cast<mlir::triton_rocm::FuncOp>(argOwner)) {
