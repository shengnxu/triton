...
using ::mlir::triton_rocm::gpu_rocm::getContigPerThread;
using ::mlir::triton_rocm::gpu_rocm::getOrder;
using ::mlir::triton_rocm::gpu_rocm::getShapePerCTA;
using ::mlir::triton_rocm::gpu_rocm::getShapePerCTATile;
using ::mlir::triton_rocm::gpu_rocm::getSizePerThread;
using ::mlir::triton_rocm::gpu_rocm::getTotalElemsPerThread;
using ::mlir::triton_rocm::gpu_rocm::isaDistributedLayout;
...
Value convertLayout(int opIdx, ConversionPatternRewriter &rewriter,
                    ...
                    TritonGPUROCMToLLVMTypeConverter *typeConverter, Value thread);
}
...
struct ConvertLayoutOpConversion
    : public ConvertTritonGPUROCMOpToLLVMPattern<triton_rocm::gpu_rocm::ConvertLayoutOp> {
public:
  using ConvertTritonGPUROCMOpToLLVMPattern<
      triton_rocm::gpu_rocm::ConvertLayoutOp>::ConvertTritonGPUROCMOpToLLVMPattern;

  LogicalResult
  matchAndRewrite(triton_rocm::gpu_rocm::ConvertLayoutOp op, OpAdaptor adaptor,
...
private:
  ...
  getMultiDimOffset(Attribute layout, Location loc,
    ...
    if (auto mmaLayout = layout.dyn_cast<MmaEncodingAttr>()) {
      ...
      unsigned iWaveSize = triton_rocm::gpu_rocm::getWarpSize(layout);
  ...
  void processReplica(Location loc, ConversionPatternRewriter &rewriter,
    ...
    bool isPtr = elemTy.isa<triton_rocm::PointerType>();
  ...
  LogicalResult
  lowerDistToDistWithDistSmem(triton_rocm::gpu_rocm::ConvertLayoutOp op,
                              OpAdaptor adaptor,
    ...
    auto srcCTAsPerCGA = triton_rocm::gpu_rocm::getCTAsPerCGA(srcLayout);
    auto srcCTAOrder = triton_rocm::gpu_rocm::getCTAOrder(srcLayout);
  ...
  LogicalResult
  lowerDistributedToDistributed(triton_rocm::gpu_rocm::ConvertLayoutOp op,
                                OpAdaptor adaptor,
                                ConversionPatternRewriter &rewriter) const {
    auto loc = op.getLoc();
    Value src = op.getSrc();
    ...
    for (unsigned repId = 0; repId < accumNumReplicates; ++repId) {
      ...
      if (repId != 0) {
        ...
        if (auto optionalAgentId = getWSAgentId(op)) {
          ...
          rewriter.create<triton_rocm::nvgpu::NamedBarrierWaitOp>(loc, bar,
                                                             kNumThreads);
        } else {
          barrier();
        }
  ...
  LogicalResult
  lowerSharedToDistributed(triton_rocm::gpu_rocm::ConvertLayoutOp op, OpAdaptor adaptor,
                           ConversionPatternRewriter &rewriter) const {
    auto loc = op.getLoc();
    Value src = op.getSrc();
    Value dst = op.getResult();
  ...
  LogicalResult
  lowerDistributedToShared(triton_rocm::gpu_rocm::ConvertLayoutOp op, OpAdaptor adaptor,
                           ConversionPatternRewriter &rewriter) const {
    auto loc = op.getLoc();
    Value src = op.getSrc();
    Value dst = op.getResult();
    ...
    auto srcShape = srcTy.getShape();
    auto dstTy = dst.getType().cast<RankedTensorType>();
    auto dstShapePerCTA = triton_rocm::gpu_rocm::getShapePerCTA(dstTy);
    ...
    if (mmaLayout && mmaLayout.isHopper() && elemSize == 16 &&
      ...
      unsigned inVec =
          inOrd == outOrd ? triton_rocm::gpu_rocm::getContigPerThread(mmaLayout)[inOrd[0]]
                          : 1;
  ...
  LogicalResult
  lowerSharedToDotOperand(triton_rocm::gpu_rocm::ConvertLayoutOp op, OpAdaptor adaptor,
                          ConversionPatternRewriter &rewriter) const {
    auto loc = op.getLoc();
    Value src = op.getSrc();
    Value dst = op.getResult();
...
#if 1
  LogicalResult
  lowerMfmaToDotOperand(triton_rocm::gpu_rocm::ConvertLayoutOp op, OpAdaptor adaptor,
                        ConversionPatternRewriter &rewriter) const {
    auto loc = op.getLoc();
  ...
  // mma -> dot_operand
  LogicalResult
  lowerMmaToDotOperand(triton_rocm::gpu_rocm::ConvertLayoutOp op, OpAdaptor adaptor,
                       ConversionPatternRewriter &rewriter) const {
    auto loc = op.getLoc();
    auto srcTy = op.getSrc().getType().cast<RankedTensorType>();
    auto dstTy = op.getResult().getType().cast<RankedTensorType>();
  ...
  Value lowerSharedToDotOperandMFMA(
      triton_rocm::gpu_rocm::ConvertLayoutOp op, OpAdaptor adaptor,
...
#endif
  // mma -> mma
  LogicalResult lowerMmaToMma(triton_rocm::gpu_rocm::ConvertLayoutOp op,
                              OpAdaptor adaptor,
                              ConversionPatternRewriter &rewriter) const {
    auto loc = op.getLoc();
    auto srcTy = op.getSrc().getType().cast<RankedTensorType>();
    auto dstTy = op.getResult().getType().cast<RankedTensorType>();
    if (triton_rocm::gpu_rocm::getTotalElemsPerThread(srcTy) ==
        triton_rocm::gpu_rocm::getTotalElemsPerThread(dstTy)) {
      rewriter.replaceOp(op, op.getSrc());
      return success();
    }
    // get source values
    ...
    SmallVector<unsigned> dstElementPerThread =
        triton_rocm::gpu_rocm::getElemsPerThread(dstTy);
    SmallVector<unsigned> srcElementPerThread =
        triton_rocm::gpu_rocm::getElemsPerThread(srcTy);
    ...
    assert(retVals.size() == triton_rocm::gpu_rocm::getTotalElemsPerThread(dstTy));
    Value view =
