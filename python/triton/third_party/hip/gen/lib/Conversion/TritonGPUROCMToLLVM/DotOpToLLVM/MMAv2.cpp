...
Value loadC(Value tensor, Value llTensor,
            TritonGPUROCMToLLVMTypeConverter *typeConverter, Location loc,
            ConversionPatternRewriter &rewriter) {
  MLIRContext *ctx = tensor.getContext();
  auto tensorTy = tensor.getType().cast<RankedTensorType>();
  size_t fcSize = triton_rocm::gpu_rocm::getTotalElemsPerThread(tensor.getType());
...
TensorCoreType getMmaType(triton_rocm::DotOp op) {
  Value A = op.getA();
  Value B = op.getB();
...
static void callMmaTuringInt8(PTXBuilder &builder, unsigned m, unsigned n,
                              unsigned k, mlir::triton_rocm::PTXInstr &mma,
...
LogicalResult convertDot(TritonGPUROCMToLLVMTypeConverter *typeConverter,
  ...
  auto aShapePerCTA = triton_rocm::gpu_rocm::getShapePerCTA(aTensorTy);
  auto bShapePerCTA = triton_rocm::gpu_rocm::getShapePerCTA(bTensorTy);
  auto dShapePerCTA = triton_rocm::gpu_rocm::getShapePerCTA(dTensorTy);
...
LogicalResult convertMMA(triton_rocm::DotOp op, triton_rocm::DotOp::Adaptor adaptor,
