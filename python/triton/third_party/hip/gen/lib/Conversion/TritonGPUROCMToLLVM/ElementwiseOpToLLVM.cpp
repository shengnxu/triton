...
static SmallVector<Value> reorderValues(const SmallVector<Value> &values,
  ...
  if (!inTensorTy || !ouTensorTy)
    return values;
  auto inEncoding =
      dyn_cast<triton_rocm::gpu_rocm::DotOperandEncodingAttr>(inTensorTy.getEncoding());
  auto ouEncoding =
      dyn_cast<triton_rocm::gpu_rocm::DotOperandEncodingAttr>(ouTensorTy.getEncoding());
  ...
  // reorder elements
  auto parentEncoding =
      dyn_cast<triton_rocm::gpu_rocm::MmaEncodingAttr>(ouEncoding.getParent());
  if (!parentEncoding)
    return values;
...
class ElementwiseOpConversionBase
    : public ConvertTritonGPUROCMOpToLLVMPattern<SourceOp> {
public:
  ...
  explicit ElementwiseOpConversionBase(
      ...
      : ConvertTritonGPUROCMOpToLLVMPattern<SourceOp>(typeConverter, benefit),
  ...
  SmallVector<Value> maybeDeduplicate(SourceOp op,
    ...
    if (!encoding)
      ...
      return resultVals;
    if (!encoding.dyn_cast<triton_rocm::gpu_rocm::BlockedEncodingAttr>() &&
        !encoding.dyn_cast<triton_rocm::gpu_rocm::SliceEncodingAttr>()) {
    ...
    SmallVector<unsigned> elemsPerThread =
        triton_rocm::gpu_rocm::getElemsPerThread(rtType);
    ...
    SmallVector<unsigned> sizePerThread =
        triton_rocm::gpu_rocm::getSizePerThread(encoding);
    ...
    if (rank > 1) {
      ...
      SmallVector<unsigned> order = triton_rocm::gpu_rocm::getOrder(encoding);
      if (rank != order.size())
        return resultVals;
...
struct FpToFpOpConversion
    : public ElementwiseOpConversionBase<triton_rocm::FpToFpOp, FpToFpOpConversion> {
  using ElementwiseOpConversionBase<
      triton_rocm::FpToFpOp, FpToFpOpConversion>::ElementwiseOpConversionBase;

  explicit FpToFpOpConversion(TritonGPUROCMToLLVMTypeConverter &typeConverter,
                              ModuleAxisInfoAnalysis &axisAnalysisPass,
  ...
  SmallVector<Value> createDestOps(triton_rocm::FpToFpOp op, OpAdaptor adaptor,
                                   ConversionPatternRewriter &rewriter,
                                   Type elemTy, MultipleOperandsRange operands,
                                   Location loc) const {
...
#define POPULATE_UNARY_OP(SRC_OP, DST_OP)                                      \
  ...
  POPULATE_UNARY_OP(triton_rocm::BitcastOp, LLVM::BitcastOp)
  POPULATE_UNARY_OP(triton_rocm::IntToPtrOp, LLVM::IntToPtrOp)
  POPULATE_UNARY_OP(triton_rocm::PtrToIntOp, LLVM::PtrToIntOp)
