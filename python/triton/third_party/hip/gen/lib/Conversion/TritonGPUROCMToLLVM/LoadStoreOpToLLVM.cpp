...
using ::mlir::triton_rocm::gpu_rocm::getTotalElemsPerThread;
...
struct LoadStoreConversionBase {
  ...
  unsigned getVectorSize(Value ptr) const {
    ...
    auto pointeeBitWidth = triton_rocm::getPointeeBitWidth(tensorTy);
...
struct LoadOpConversion
    : public ConvertTritonGPUROCMOpToLLVMPattern<triton_rocm::LoadOp>,
      public LoadStoreConversionBase {
  using ConvertTritonGPUROCMOpToLLVMPattern<
      triton_rocm::LoadOp>::ConvertTritonGPUROCMOpToLLVMPattern;

  LoadOpConversion(TritonGPUROCMToLLVMTypeConverter &converter,
                   ModuleAxisInfoAnalysis &axisAnalysisPass,
                   PatternBenefit benefit)
      : ConvertTritonGPUROCMOpToLLVMPattern<triton_rocm::LoadOp>(converter, benefit),
        LoadStoreConversionBase(axisAnalysisPass) {}

  LogicalResult
  matchAndRewrite(triton_rocm::LoadOp op, OpAdaptor adaptor,
    ...
    for (size_t vecStart = 0; vecStart < numElems; vecStart += vec) {
      ...
      auto &ld = ptxBuilder.create<>("ld")
                     ...
                     .global()
                     .o("ca", op.getCache() == triton_rocm::CacheModifier::CA)
                     .o("cg", op.getCache() == triton_rocm::CacheModifier::CG)
                     .o("L1::evict_first",
                        op.getEvict() == triton_rocm::EvictionPolicy::EVICT_FIRST)
                     .o("L1::evict_last",
                        op.getEvict() == triton_rocm::EvictionPolicy::EVICT_LAST)
...
struct StoreOpConversion
    : public ConvertTritonGPUROCMOpToLLVMPattern<triton_rocm::StoreOp>,
      public LoadStoreConversionBase {
  using ConvertTritonGPUROCMOpToLLVMPattern<
      triton_rocm::StoreOp>::ConvertTritonGPUROCMOpToLLVMPattern;

  StoreOpConversion(TritonGPUROCMToLLVMTypeConverter &converter,
                    ModuleAxisInfoAnalysis &axisAnalysisPass,
                    PatternBenefit benefit)
      : ConvertTritonGPUROCMOpToLLVMPattern<triton_rocm::StoreOp>(converter, benefit),
        LoadStoreConversionBase(axisAnalysisPass) {}

  LogicalResult
  matchAndRewrite(triton_rocm::StoreOp op, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    Value ptr = op.getPtr();
    ...
    for (size_t vecStart = 0; vecStart < elemsPerThread; vecStart += vec) {
      ...
      auto &ptxStoreInstr =
          ptxBuilder.create<>("st")
              ->global()
              .o("wb", op.getCache() == triton_rocm::CacheModifier::WB)
              .o("cg", op.getCache() == triton_rocm::CacheModifier::CG)
              .o("cs", op.getCache() == triton_rocm::CacheModifier::CS)
              .o("wt", op.getCache() == triton_rocm::CacheModifier::WT)
              .o("L1::evict_first",
                 op.getEvict() == triton_rocm::EvictionPolicy::EVICT_FIRST)
              .o("L1::evict_last",
                 op.getEvict() == triton_rocm::EvictionPolicy::EVICT_LAST)
...
struct StoreAsyncOpConversion
    : public ConvertTritonGPUROCMOpToLLVMPattern<triton_rocm::nvidia_gpu::StoreAsyncOp> {
  using ConvertTritonGPUROCMOpToLLVMPattern<
      triton_rocm::nvidia_gpu::StoreAsyncOp>::ConvertTritonGPUROCMOpToLLVMPattern;

  StoreAsyncOpConversion(TritonGPUROCMToLLVMTypeConverter &converter,
                         ModuleAllocation &allocation,
                         mlir::triton_rocm::gpu_rocm::TMAMetadataTy *tmaMetadata,
                         const TensorPtrMapT *tensorPtrMap,
                         PatternBenefit benefit)
      : ConvertTritonGPUROCMOpToLLVMPattern<triton_rocm::nvidia_gpu::StoreAsyncOp>(
            converter, allocation, tmaMetadata, benefit),
        tensorPtrMap(tensorPtrMap) {}

  LogicalResult
  matchAndRewrite(triton_rocm::nvidia_gpu::StoreAsyncOp op, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
  ...
  LogicalResult lowerStoreAsync(triton_rocm::nvidia_gpu::StoreAsyncOp op,
                                OpAdaptor adaptor,
    ...
    mlir::triton_rocm::gpu_rocm::TMAInfo tmaInfo;
    ...
    rewriter.create<triton_rocm::nvgpu::FenceAsyncSharedOp>(loc, 0);
    ...
    for (uint32_t b = 0; b < numBox; ++b) {
      ...
      rewriter.create<triton_rocm::nvgpu::TMAStoreTiledOp>(loc, tmaDesc, addr, pred,
                                                      coord);
    }
    rewriter.eraseOp(op);
    return success();
  ...
  LogicalResult
  lowerStoreAsyncWithSlice(triton_rocm::nvidia_gpu::StoreAsyncOp op,
                           OpAdaptor adaptor,
                           ConversionPatternRewriter &rewriter) const {
    auto loc = op.getLoc();
    MLIRContext *ctx = rewriter.getContext();
    ...
    unsigned numElems = triton_rocm::gpu_rocm::getTotalElemsPerThread(srcTy);
    ...
    auto srcOrder = triton_rocm::gpu_rocm::getOrder(srcLayout);
    unsigned inVec =
        srcOrder == sharedLayout.getOrder()
            ? triton_rocm::gpu_rocm::getContigPerThread(srcLayout)[srcOrder[0]]
            : 1;
    ...
    for (uint32_t b = 0; b < numBox; ++b) {
      for (int rep = 0; rep < repM; ++rep) {
        ...
        for (unsigned idx = 0; idx < numElemsPerRep / numBox; idx += 8) {
          ...
          Value offset = rewriter.create<triton_rocm::nvgpu::OffsetOfStmatrixV4Op>(
          ...
          rewriter.create<triton_rocm::nvgpu::StoreMatrixOp>(
        ...
        auto addr = bitcast(srcPtrBase, ptrI8SharedTy);
        rewriter.create<triton_rocm::nvgpu::TMAStoreTiledOp>(loc, tmaDesc, addr,
                                                        pred, coord);
      }
    }
    rewriter.eraseOp(op);
...
void createBarrier(ConversionPatternRewriter &rewriter, Location loc,
  ...
  if (numCTAs == 1) {
    ...
    rewriter.create<triton_rocm::nvidia_gpu::ClusterArriveOp>(loc, false);
    rewriter.create<triton_rocm::nvidia_gpu::ClusterWaitOp>(loc);
  }
#endif
}
} // namespace
...
struct AtomicCASOpConversion
    : public ConvertTritonGPUROCMOpToLLVMPattern<triton_rocm::AtomicCASOp>,
      public LoadStoreConversionBase {
  using ConvertTritonGPUROCMOpToLLVMPattern<
      triton_rocm::AtomicCASOp>::ConvertTritonGPUROCMOpToLLVMPattern;

  AtomicCASOpConversion(TritonGPUROCMToLLVMTypeConverter &converter,
                        ModuleAllocation &allocation,
                        ModuleAxisInfoAnalysis &axisAnalysisPass,
                        PatternBenefit benefit)
      : ConvertTritonGPUROCMOpToLLVMPattern<triton_rocm::AtomicCASOp>(
            converter, allocation, benefit),
        LoadStoreConversionBase(axisAnalysisPass) {}

#if 1
  LogicalResult
  matchAndRewrite(triton_rocm::AtomicCASOp op, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
  ...
  matchAndRewrite(triton_rocm::AtomicCASOp op, OpAdaptor adaptor,
    ...
    int numCTAs = triton_rocm::gpu_rocm::TritonGPUROCMDialect::getNumCTAs(moduleOp);
...
struct AtomicRMWOpConversion
    : public ConvertTritonGPUROCMOpToLLVMPattern<triton_rocm::AtomicRMWOp>,
      public LoadStoreConversionBase {
  using ConvertTritonGPUROCMOpToLLVMPattern<
      triton_rocm::AtomicRMWOp>::ConvertTritonGPUROCMOpToLLVMPattern;

  AtomicRMWOpConversion(TritonGPUROCMToLLVMTypeConverter &converter,
                        ModuleAllocation &allocation,
                        ModuleAxisInfoAnalysis &axisAnalysisPass,
                        PatternBenefit benefit)
      : ConvertTritonGPUROCMOpToLLVMPattern<triton_rocm::AtomicRMWOp>(
            converter, allocation, benefit),
        LoadStoreConversionBase(axisAnalysisPass) {}
  ...
  LogicalResult
  matchAndRewrite(triton_rocm::AtomicRMWOp op, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    auto loc = op.getLoc();
    MLIRContext *ctx = rewriter.getContext();
...
struct InsertSliceOpConversion
    : public ConvertTritonGPUROCMOpToLLVMPattern<tensor::InsertSliceOp> {
  using ConvertTritonGPUROCMOpToLLVMPattern<
      tensor::InsertSliceOp>::ConvertTritonGPUROCMOpToLLVMPattern;
...
struct InsertSliceAsyncOpConversion
    : public ConvertTritonGPUROCMOpToLLVMPattern<triton_rocm::gpu_rocm::InsertSliceAsyncOp>,
      public LoadStoreConversionBase {
  using ConvertTritonGPUROCMOpToLLVMPattern<
      triton_rocm::gpu_rocm::InsertSliceAsyncOp>::ConvertTritonGPUROCMOpToLLVMPattern;

  InsertSliceAsyncOpConversion(
      TritonGPUROCMToLLVMTypeConverter &converter, ModuleAllocation &allocation,
      ...
      : ConvertTritonGPUROCMOpToLLVMPattern<triton_rocm::gpu_rocm::InsertSliceAsyncOp>(
            converter, allocation, indexCacheInfo, benefit),
        LoadStoreConversionBase(axisAnalysisPass) {}

  LogicalResult
  matchAndRewrite(triton_rocm::gpu_rocm::InsertSliceAsyncOp op, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
...
struct InsertSliceAsyncV2OpConversion
    : public ConvertTritonGPUROCMOpToLLVMPattern<
          triton_rocm::nvidia_gpu::InsertSliceAsyncV2Op> {
  using ConvertTritonGPUROCMOpToLLVMPattern<
      triton_rocm::nvidia_gpu::InsertSliceAsyncV2Op>::
      ConvertTritonGPUROCMOpToLLVMPattern;

  InsertSliceAsyncV2OpConversion(TritonGPUROCMToLLVMTypeConverter &converter,
                                 ...
                                 PatternBenefit benefit)
      : ConvertTritonGPUROCMOpToLLVMPattern<
            triton_rocm::nvidia_gpu::InsertSliceAsyncV2Op>(converter, allocation,
                                                      tmaMetadata, benefit),
        tensorPtrMap(tensorPtrMap) {}

  LogicalResult
  matchAndRewrite(triton_rocm::nvidia_gpu::InsertSliceAsyncV2Op op,
                  OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    ...
    for (size_t i = 0; i < numBoxes; ++i) {
      ...
      rewriter.create<triton_rocm::nvgpu::TMALoadTiledOp>(
