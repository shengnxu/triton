...
struct ReduceOpConversion
    : public ConvertTritonGPUROCMOpToLLVMPattern<triton_rocm::ReduceOp> {
public:
  ReduceOpConversion(
      ...
      : ConvertTritonGPUROCMOpToLLVMPattern<triton_rocm::ReduceOp>(
...
private:
  ...
  void accumulate(ConversionPatternRewriter &rewriter, Region &combineOp,
    ...
    auto returnOp = dyn_cast<triton_rocm::ReduceReturnOp>(newReduce.getTerminator());
  ...
  unpackInputs(Location loc, triton_rocm::ReduceOp op, OpAdaptor adaptor,
  ...
  SmallVector<Value> getSmemBases(ReduceOpHelper &helper, triton_rocm::ReduceOp op,
  ...
  std::optional<NVVM::ReduxKind> matchReduxKind(triton_rocm::ReduceOp op) const {
  #if 1
    return std::nullopt;
  #endif
  ...
  void reduceWithinThreads(
      ...
      std::map<SmallVector<unsigned>, SmallVector<Value>> &indices,
      ConversionPatternRewriter &rewriter) const {
    triton_rocm::ReduceOp op = helper.getOperation();
  ...
  void warpReduce(ConversionPatternRewriter &rewriter, Location loc,
                  SmallVector<Value> &acc, triton_rocm::ReduceOp op,
    ...
    for (unsigned N = numLaneToReduce / 2; N > 0; N >>= 1) {
      ...
      auto inMfma =
        inputTy.getEncoding().dyn_cast<triton_rocm::gpu_rocm::MfmaEncodingAttr>();
  ...
  Type getElementPtrType(triton_rocm::ReduceOp op, int i) const {
  ...
  getMultiDimWarpId(ReduceOpHelper &helper, Value &warpId, Location &loc,
    ...
    if (auto sliceLayout = srcLayout.dyn_cast<SliceEncodingAttr>()) {
      ...
      auto parentWarpsPerCTA = triton_rocm::gpu_rocm::getWarpsPerCTA(parentLayout);
      auto parentOrder = triton_rocm::gpu_rocm::getOrder(parentLayout);
      multiDimWarpId =
      ...
      auto warpsPerCTA =
          triton_rocm::gpu_rocm::getWarpsPerCTAWithUniqueData(srcLayout, srcShape);
  ...
  void storeWarpReduceToSharedMemory(
    ...
    Value threadId = getThreadId(rewriter, loc);
    auto srcLayout = helper.getSrcLayout();
    unsigned wavefront_size = triton_rocm::gpu_rocm::getWarpSize(srcLayout);
    ...
    auto threadsPerWarp =
        triton_rocm::gpu_rocm::getThreadsPerWarpWithUniqueData(srcLayout, srcShape);
    auto order = getOrder(srcLayout);
  ...
  void accumulatePartialReductions(ReduceOpHelper &helper,
    ...
    unsigned numThreads =
        product<unsigned>(triton_rocm::gpu_rocm::getWarpsPerCTA(srcLayout)) *
        triton_rocm::gpu_rocm::TritonGPUROCMDialect::getThreadsPerWarp(mod);
