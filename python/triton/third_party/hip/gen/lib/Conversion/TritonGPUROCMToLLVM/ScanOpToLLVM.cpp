...
static Value accumulate(ConversionPatternRewriter &rewriter, Region &combineOp,
  ...
  auto returnOp = dyn_cast<triton_rocm::ScanReturnOp>(newScan.getTerminator());
...
struct ScanOpConversion
    : public ConvertTritonGPUROCMOpToLLVMPattern<triton_rocm::ScanOp> {
public:
  using ConvertTritonGPUROCMOpToLLVMPattern<
      triton_rocm::ScanOp>::ConvertTritonGPUROCMOpToLLVMPattern;
...
private:
  ...
  getDelinearizedIds(ConversionPatternRewriter &rewriter,
                     ScanLoweringHelper &helper, Value laneId,
                     Value warpId) const;
  LogicalResult emitFastScan(triton_rocm::ScanOp op, triton_rocm::ScanOpAdaptor adaptor,
...
ScanOpConversion::getMultiDimLaneId(ConversionPatternRewriter &rewriter,
  ...
  auto threadsPerWarp = triton_rocm::gpu_rocm::getThreadsPerWarp(srcEncoding);
  auto warpsPerCTA = triton_rocm::gpu_rocm::getWarpsPerCTA(srcEncoding);
  auto order = triton_rocm::gpu_rocm::getOrder(srcEncoding);
...
LogicalResult
ScanOpConversion::emitFastScan(triton_rocm::ScanOp op, triton_rocm::ScanOpAdaptor adaptor,
                               ConversionPatternRewriter &rewriter) const {
  ...
  unsigned iWarpSize = triton_rocm::gpu_rocm::TritonGPUROCMDialect::getThreadsPerWarp(mod);
  ...
  if (axisNumWarps > 1) {
    ...
    auto threadsPerWarp = triton_rocm::gpu_rocm::getThreadsPerWarp(helper.getEncoding());
    auto laneIdLast = linearize(rewriter, loc, multiDimLaneId, threadsPerWarp,
                                triton_rocm::gpu_rocm::getOrder(helper.getEncoding()));
