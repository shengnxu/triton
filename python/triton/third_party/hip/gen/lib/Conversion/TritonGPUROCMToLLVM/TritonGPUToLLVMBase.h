#ifndef TRITON_CONVERSION_ROCM_TRITONGPU_TO_LLVM_BASE_H
#define TRITON_CONVERSION_ROCM_TRITONGPU_TO_LLVM_BASE_H
...
using ::mlir::triton_rocm::gpu_rocm::CTALayoutAttr;
using ::mlir::triton_rocm::gpu_rocm::DotOperandEncodingAttr;
using ::mlir::triton_rocm::gpu_rocm::MfmaEncodingAttr;
using ::mlir::triton_rocm::gpu_rocm::MmaEncodingAttr;
using ::mlir::triton_rocm::gpu_rocm::SliceEncodingAttr;
using ::mlir::triton_rocm::gpu_rocm::TMAMetadataTy;
namespace ttng = ::mlir::triton_rocm::nvidia_gpu;

typedef DenseMap<Operation *, triton_rocm::MakeTensorPtrOp> TensorPtrMapT;
...
struct FuncOpConversionBase : public ConvertOpToLLVMPattern<triton_rocm::FuncOp> {
protected:
  /// Only retain those attributes that are not constructed by
  /// `LLVMFuncOp::build`. If `filterArgAttrs` is set, also filter out argument
  /// attributes.
  static void filterFuncAttributes(triton_rocm::FuncOp op, bool filterArgAttrs,
...
protected:
  using ConvertOpToLLVMPattern<triton_rocm::FuncOp>::ConvertOpToLLVMPattern;

  // Convert input FuncOp to LLVMFuncOp by using the LLVMTypeConverter provided
  // to this legalization pattern.
  LLVM::LLVMFuncOp
  convertFuncOpToLLVMFuncOp(triton_rocm::FuncOp funcOp,
...
class ConvertTritonGPUROCMOpToLLVMPatternBase {
public:
  ...
  explicit ConvertTritonGPUROCMOpToLLVMPatternBase(
  ...
  explicit ConvertTritonGPUROCMOpToLLVMPatternBase(
      TritonGPUROCMToLLVMTypeConverter &typeConverter, ModuleAllocation &allocation)
  ...
  TritonGPUROCMToLLVMTypeConverter *getTypeConverter() const { return converter; }
  ...
  Value getClusterCTAId(ConversionPatternRewriter &rewriter,
                        Location loc) const {
    return rewriter.create<triton_rocm::nvgpu::ClusterCTAIdOp>(
  ...
  getSwizzledSharedPtrs(Location loc, unsigned inVec, RankedTensorType srcTy,
                        triton_rocm::gpu_rocm::SharedEncodingAttr resSharedLayout,
    ...
    auto srcShapePerCTA = triton_rocm::gpu_rocm::getShapePerCTA(srcTy);
    ...
    // Order
    auto inOrder = triton_rocm::gpu_rocm::getOrder(srcEncoding);
    auto outOrder = triton_rocm::gpu_rocm::getOrder(resSharedLayout);
    assert(maxPhase == 1 ||
  ...
  loadSharedToDistributed(Value dst, ArrayRef<SmallVector<Value>> dstIndices,
    ...
    auto srcSharedLayout =
        srcTy.getEncoding().cast<triton_rocm::gpu_rocm::SharedEncodingAttr>();
    auto srcElemTy = srcTy.getElementType();
    auto dstElemTy = dstTy.getElementType();
    auto inOrd = triton_rocm::gpu_rocm::getOrder(srcSharedLayout);
    auto outOrd = triton_rocm::gpu_rocm::getOrder(dstDistributedLayout);
    unsigned outVec = inOrd == outOrd
                          ? triton_rocm::gpu_rocm::getUniqueContigPerThread(
                                dstDistributedLayout, dstShape)[outOrd[0]]
                          : 1;
    unsigned inVec = srcSharedLayout.getVec();
    unsigned minVec = std::min(outVec, inVec);
    unsigned outElems = triton_rocm::gpu_rocm::getTotalElemsPerThread(dstTy);
  ...
  void storeDistributedToShared(Value src, Value llSrc,
    ...
    auto dstSharedLayout =
        dstTy.getEncoding().cast<triton_rocm::gpu_rocm::SharedEncodingAttr>();
    auto dstElemTy = dstTy.getElementType();
    auto inOrd = triton_rocm::gpu_rocm::getOrder(srcDistributedLayout);
  ...
  Value getMask(Type valueTy, ConversionPatternRewriter &rewriter,
    ...
    if (tensorTy) {
      ...
      auto sizePerThread = triton_rocm::gpu_rocm::getSizePerThread(layout);
      auto threadsPerWarp = triton_rocm::gpu_rocm::getThreadsPerWarp(layout);
      auto warpsPerCTA = triton_rocm::gpu_rocm::getWarpsPerCTA(layout);
      auto order = triton_rocm::gpu_rocm::getOrder(layout);
      auto shapePerCTATile = triton_rocm::gpu_rocm::getShapePerCTATile(layout, shape);
      Value warpSize = i32_val(triton_rocm::gpu_rocm::getWarpSize(layout));
      ...
      if (triton_rocm::gpu_rocm::getNumCTAs(layout) > 1) {
        auto _0 = i32_val(0);
        auto CTAsPerCGA = triton_rocm::gpu_rocm::getCTAsPerCGA(layout);
        auto CTASplitNum = triton_rocm::gpu_rocm::getCTASplitNum(layout);
        auto CTAOrder = triton_rocm::gpu_rocm::getCTAOrder(layout);
  ...
  SmallVector<Value> emitCTAOffsetForLayout(Location loc,
    ...
    unsigned rank = shape.size();
    SmallVector<unsigned> CTAsPerCGA = triton_rocm::gpu_rocm::getCTAsPerCGA(layout);
    SmallVector<unsigned> CTASplitNum = triton_rocm::gpu_rocm::getCTASplitNum(layout);
    SmallVector<unsigned> CTAOrder = triton_rocm::gpu_rocm::getCTAOrder(layout);
    SmallVector<int64_t> shapePerCTA =
        triton_rocm::gpu_rocm::getShapePerCTA(CTASplitNum, shape);
...
private:
  ...
  SmallVector<Value> emitBaseIndexWithinCTAForBlockedLayout(
    ...
    Value warpSize = i32_val(triton_rocm::gpu_rocm::getWarpSize(blockedLayout));
    ...
    auto shapePerCTA = triton_rocm::gpu_rocm::getShapePerCTA(blockedLayout, shape);
    unsigned rank = shape.size();
  ...
  emitOffsetForBlockedLayout(const BlockedEncodingAttr &blockedLayout,
    ...
    unsigned elemsPerThread = triton_rocm::gpu_rocm::getTotalElemsPerThread(type);
  ...
  SmallVector<Value> emitBaseIndexWithinCTAForMmaLayoutV1(
    ...
    Value warpSize = i32_val(triton_rocm::gpu_rocm::getWarpSize(mmaLayout));
  ...
  SmallVector<Value> emitBaseIndexWithinCTAForMmaLayoutV2V3(
    ...
    auto order = triton_rocm::gpu_rocm::getOrder(mmaLayout);
  ...
  emitBaseIndexForMfmaLayout(Location loc, ConversionPatternRewriter &rewriter,
    ...
    Value threadId = getThreadId(rewriter, loc);
    Value warpSize = i32_val(triton_rocm::gpu_rocm::getWarpSize(mfmaLayout));
...
class ConvertTritonGPUROCMOpToLLVMPattern
    : public ConvertOpToLLVMPattern<SourceOp>,
      public ConvertTritonGPUROCMOpToLLVMPatternBase {
public:
  using OpAdaptor = typename SourceOp::Adaptor;

  explicit ConvertTritonGPUROCMOpToLLVMPattern(
      TritonGPUROCMToLLVMTypeConverter &typeConverter, PatternBenefit benefit = 1)
      : ConvertOpToLLVMPattern<SourceOp>(typeConverter, benefit),
        ConvertTritonGPUROCMOpToLLVMPatternBase(typeConverter) {}

  explicit ConvertTritonGPUROCMOpToLLVMPattern(
      TritonGPUROCMToLLVMTypeConverter &typeConverter, ModuleAllocation &allocation,
      PatternBenefit benefit = 1)
      : ConvertOpToLLVMPattern<SourceOp>(typeConverter, benefit),
        ConvertTritonGPUROCMOpToLLVMPatternBase(typeConverter, allocation) {}

  explicit ConvertTritonGPUROCMOpToLLVMPattern(
      TritonGPUROCMToLLVMTypeConverter &typeConverter,
      IndexCacheInfo indexCacheInfo, PatternBenefit benefit = 1)
      : ConvertOpToLLVMPattern<SourceOp>(typeConverter, benefit),
        ConvertTritonGPUROCMOpToLLVMPatternBase(typeConverter, indexCacheInfo) {}
  ...
  explicit ConvertTritonGPUROCMOpToLLVMPattern(
      TritonGPUROCMToLLVMTypeConverter &typeConverter, ModuleAllocation &allocation,
      mlir::triton_rocm::gpu_rocm::TMAMetadataTy *tmaMetadata, PatternBenefit benefit = 1)
      : ConvertOpToLLVMPattern<SourceOp>(typeConverter, benefit),
        ConvertTritonGPUROCMOpToLLVMPatternBase(typeConverter, allocation,
                                            tmaMetadata) {}

protected:
  TritonGPUROCMToLLVMTypeConverter *getTypeConverter() const {
    LLVMTypeConverter *ret =
        ((ConvertTritonGPUROCMOpToLLVMPatternBase *)this)->getTypeConverter();
    return (TritonGPUROCMToLLVMTypeConverter *)ret;
  }
};
