...
public:
  FoldSplatMaskInInsertAsync(mlir::MLIRContext *context)
      : mlir::RewritePattern(
            triton_rocm::nvidia_gpu::InsertSliceAsyncV2Op::getOperationName(), 1,
            context) {}

  LogicalResult
  matchAndRewrite(mlir::Operation *op,
                  mlir::PatternRewriter &rewriter) const override {
    auto insertOp = cast<triton_rocm::nvidia_gpu::InsertSliceAsyncV2Op>(op);
    if (!insertOp.getMask())
      return failure();
    auto splatOp = insertOp.getMask().getDefiningOp<triton_rocm::SplatOp>();
    if (!splatOp)
      return failure();
...
struct ReturnOpConversion : public ConvertOpToLLVMPattern<triton_rocm::ReturnOp> {
  using ConvertOpToLLVMPattern<triton_rocm::ReturnOp>::ConvertOpToLLVMPattern;

  LogicalResult
  matchAndRewrite(triton_rocm::ReturnOp op, OpAdaptor adaptor,
...
struct FuncOpConversion : public FuncOpConversionBase {
  ...
  triton_rocm::FuncOp amendFuncOp(triton_rocm::FuncOp funcOp,
    ...
    auto amendedFuncOp = rewriter.create<triton_rocm::FuncOp>(
  ...
  LogicalResult
  matchAndRewrite(triton_rocm::FuncOp funcOp, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    ...
    funcOp.walk(
        [&numTMALoad](triton_rocm::nvidia_gpu::InsertSliceAsyncV2Op insertSliceOp) {
          numTMALoad++;
        });
    unsigned numTMAStore = 0;
    funcOp.walk([&numTMAStore](triton_rocm::nvidia_gpu::StoreAsyncOp storeAsyncOp) {
      numTMAStore++;
    });
...
struct CallOpConversion : public ConvertOpToLLVMPattern<triton_rocm::CallOp> {
  CallOpConversion(LLVMTypeConverter &converter, int numWarps,
                   ModuleAllocation &allocation, PatternBenefit benefit)
      : ConvertOpToLLVMPattern<triton_rocm::CallOp>(converter, benefit),
        numWarps(numWarps), allocation(allocation) {}

  LogicalResult
  matchAndRewrite(triton_rocm::CallOp callOp,
                  typename triton_rocm::CallOp::Adaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
...
private:
  SmallVector<Value, 4>
  promoteOperands(triton_rocm::CallOp callOp,
                  typename triton_rocm::CallOp::Adaptor adaptor,
                  ConversionPatternRewriter &rewriter) const {
  ...
  LLVM::CallOp
  convertCallOpToLLVMCallOp(triton_rocm::CallOp callOp,
  ...
  SmallVector<Value>
  getCallOpResults(triton_rocm::CallOp callOp, LLVM::CallOp newCallOp,
                   ConversionPatternRewriter &rewriter) const {
...
public:
  explicit TritonLLVMConversionTarget(MLIRContext &ctx, Target target)
    ...
    addLegalDialect<mlir::triton_rocm::nvgpu::NVGPUROCMDialect>();
    addIllegalDialect<triton_rocm::TritonROCMDialect>();
    addIllegalDialect<triton_rocm::gpu_rocm::TritonGPUROCMDialect>();
    addIllegalDialect<triton_rocm::nvidia_gpu::TritonNvidiaGPUROCMDialect>();
...
struct ConvertTritonGPUROCMToLLVM
    : public triton_rocm::impl::ConvertTritonGPUROCMToLLVMBase<ConvertTritonGPUROCMToLLVM> {
  using ConvertTritonGPUROCMToLLVMBase<
      ConvertTritonGPUROCMToLLVM>::ConvertTritonGPUROCMToLLVMBase;

  void getDependentDialects(DialectRegistry &registry) const override {
    registry.insert<triton_rocm::nvgpu::NVGPUROCMDialect, LLVM::LLVMDialect,
                    NVVM::NVVMDialect, ROCDL::ROCDLDialect>();
  }

  ConvertTritonGPUROCMToLLVM(int32_t computeCapability, Target target,
                         mlir::triton_rocm::gpu_rocm::TMAMetadataTy *tmaMetadata)
      : ConvertTritonGPUROCMToLLVMBase({computeCapability, target}),
  ...
  void runOnOperation() override {
    ...
    TritonGPUROCMToLLVMTypeConverter typeConverter(context, option);
    TritonLLVMConversionTarget convTarget(*context, target);
    int numWarps = triton_rocm::gpu_rocm::TritonGPUROCMDialect::getNumWarps(mod);
    int numCTAs = triton_rocm::gpu_rocm::TritonGPUROCMDialect::getNumCTAs(mod);
    int threadsPerWarp = triton_rocm::gpu_rocm::TritonGPUROCMDialect::getThreadsPerWarp(mod);
    ...
    mod.walk([&tensorPtrMap](
                 mlir::triton_rocm::nvidia_gpu::InsertSliceAsyncV2Op insertOp) {
      auto src = insertOp.getSrc();
      auto ptrTy = src.getType().dyn_cast<triton_rocm::PointerType>();
    ...
    mod.walk([&tensorPtrMap](mlir::triton_rocm::nvidia_gpu::StoreAsyncOp storeOp) {
      auto dst = storeOp.getDst();
      auto ptrTy = dst.getType().dyn_cast<triton_rocm::PointerType>();
      if (ptrTy && ptrTy.getPointeeType().isa<RankedTensorType>()) {
    ...
    // Hack: cleanup
    {
      ...
      mod.walk([&insertSlices](triton_rocm::nvidia_gpu::InsertSliceAsyncV2Op op) {
    ...
    bool isWarpSpecialization =
        ttng::TritonNvidiaGPUROCMDialect::getWSSupportedAttr(mod);
    ...
    mlir::triton_rocm::gpu_rocm::TMAMetadataTy tmaMetaDataDebug;
    ...
    populatePatterns1(populateTritonGPUROCMToLLVMPatterns);
    ...
    if (numCTAs == 1) {
      mod.walk([](triton_rocm::nvgpu::ClusterCTAIdOp id) {
        OpBuilder b(id);
...
private:
  ...
  DenseMap<IndexCacheKeyT, SmallVector<SmallVector<Value>>,
      ...
      indexCache;
  mlir::triton_rocm::gpu_rocm::TMAMetadataTy *tmaMetadata = nullptr;
  ...
  void decomposeFp8e4b15Convert(ModuleOp mod) const {
    mod.walk([&](triton_rocm::gpu_rocm::ConvertLayoutOp cvtOp) -> void {
      ...
      if (argEncoding.isa<triton_rocm::gpu_rocm::DotOperandEncodingAttr>() ||
          cvtEncoding.isa<triton_rocm::gpu_rocm::DotOperandEncodingAttr>())
        return;
      ...
      auto newArg = builder.create<mlir::triton_rocm::FpToFpOp>(
          cvtOp.getLoc(), newArgType, cvtOp.getOperand());
      addWSNamedAttrs(newArg, cvtOp->getAttrs());
      auto newCvt = builder.create<mlir::triton_rocm::gpu_rocm::ConvertLayoutOp>(
          cvtOp.getLoc(), newCvtType, newArg);
      addWSNamedAttrs(newCvt, cvtOp->getAttrs());
      auto newRet = builder.create<mlir::triton_rocm::FpToFpOp>(
  ...
  void decomposeMmaToDotOperand(ModuleOp mod, int numWarps, int threadsPerWarp,
    ...
    mod.walk([&](triton_rocm::gpu_rocm::ConvertLayoutOp cvtOp) -> void {
      ...
      auto srcMma =
          srcType.getEncoding().dyn_cast<triton_rocm::gpu_rocm::MmaEncodingAttr>();
      auto dstDotOp =
          dstType.getEncoding().dyn_cast<triton_rocm::gpu_rocm::DotOperandEncodingAttr>();
      if (srcMma && dstDotOp && !isMmaToDotShortcut(srcType, dstType)) {
        ...
        auto tmp = builder.create<triton_rocm::gpu_rocm::ConvertLayoutOp>(
            cvtOp.getLoc(), tmpType, cvtOp.getOperand());
        addWSNamedAttrs(tmp, cvtOp->getAttrs());
        auto newConvert = builder.create<triton_rocm::gpu_rocm::ConvertLayoutOp>(
  ...
  void decomposeMfmaToDotOperand(ModuleOp mod, int numWarps, int threadsPerWarp,
    ...
    mod.walk([&](triton_rocm::gpu_rocm::ConvertLayoutOp cvtOp) -> void {
      ...
      auto dstType = cvtOp.getType().cast<RankedTensorType>();
      auto srcMfma =
          srcType.getEncoding().dyn_cast<triton_rocm::gpu_rocm::MfmaEncodingAttr>();
      auto dstDotOp =
          dstType.getEncoding().dyn_cast<triton_rocm::gpu_rocm::DotOperandEncodingAttr>();
  ...
  int getCvtOpLDSUsage(triton_rocm::gpu_rocm::ConvertLayoutOp &cvtOp) const {
    unsigned inVec = 0;
    unsigned outVec = 0;
    ...
    auto srcType = cvtOp.getOperand().getType().cast<RankedTensorType>();
    auto bytes =
        srcType.getElementType().isa<triton_rocm::PointerType>()
  ...
  std::pair<triton_rocm::gpu_rocm::ConvertLayoutOp, triton_rocm::gpu_rocm::ConvertLayoutOp>
  createNewConvertOps(ModuleOp &mod, OpBuilder &builder,
                      triton_rocm::gpu_rocm::ConvertLayoutOp &cvtOp,
    ...
    auto srcMfma =
        srcType.getEncoding().dyn_cast<triton_rocm::gpu_rocm::MfmaEncodingAttr>();
    auto newMfmaEnc = triton_rocm::gpu_rocm::MfmaEncodingAttr::get(
    ...
    auto tmpCvt = builder.create<triton_rocm::gpu_rocm::ConvertLayoutOp>(
        cvtOp.getLoc(), newSrcType, cvtOp.getOperand());
    auto newEpilogueCvt = builder.create<triton_rocm::gpu_rocm::ConvertLayoutOp>(
  ...
  void reduceCvtOpLDSUsage(ModuleOp mod) const {
    mod.walk([&](triton_rocm::gpu_rocm::ConvertLayoutOp cvtOp) -> void {
      ...
      auto srcMfma =
          srcType.getEncoding().dyn_cast<triton_rocm::gpu_rocm::MfmaEncodingAttr>();
      auto dstBlocked =
          dstType.getEncoding().dyn_cast<triton_rocm::gpu_rocm::BlockedEncodingAttr>();
      ...
      triton_rocm::gpu_rocm::ConvertLayoutOp tmpCvt;
      triton_rocm::gpu_rocm::ConvertLayoutOp newEpilogueCvt;
  ...
  void decomposeBlockedToDotOperand(ModuleOp mod) const {
    ...
    mod.walk([&](triton_rocm::gpu_rocm::ConvertLayoutOp cvtOp) -> void {
      ...
      auto dstType = cvtOp.getType().cast<RankedTensorType>();
      auto srcBlocked =
          srcType.getEncoding().dyn_cast<triton_rocm::gpu_rocm::BlockedEncodingAttr>();
      auto dstDotOp =
          dstType.getEncoding().dyn_cast<triton_rocm::gpu_rocm::DotOperandEncodingAttr>();
      if (srcBlocked && dstDotOp) {
        auto tmpType = RankedTensorType::get(
            dstType.getShape(), dstType.getElementType(),
            triton_rocm::gpu_rocm::SharedEncodingAttr::get(
  ...
  void decomposeInsertSliceAsyncOp(ModuleOp mod) const {
    ...
    mod.walk([&](triton_rocm::gpu_rocm::InsertSliceAsyncOp insertSliceAsyncOp) -> void {
      ...
      auto resSharedLayout =
          dstTy.getEncoding().dyn_cast<triton_rocm::gpu_rocm::SharedEncodingAttr>();
...
#if 0
      if (triton_rocm::gpu_rocm::InsertSliceAsyncOp::getEligibleLoadByteWidth(
              computeCapability)
              .contains(byteWidth)) {
        return;
      }
      ...
      auto loadOp = builder.create<triton_rocm::LoadOp>(
    ...
    mod.walk([&](triton_rocm::gpu_rocm::AsyncCommitGroupOp asyncCommitGroupOp) -> void {
#if 1
      asyncCommitGroupOp.erase();
#else
      if (!triton_rocm::gpu_rocm::AsyncCommitGroupOp::isSupported(computeCapability))
        asyncCommitGroupOp.erase();
#endif
    });

    mod.walk([&](triton_rocm::gpu_rocm::AsyncWaitOp asyncWaitOp) -> void {
#if 1
      // AsyncWait is not supported for ROCM and should be removed
      asyncWaitOp.erase();
#else
      if (!triton_rocm::gpu_rocm::AsyncWaitOp::isSupported(computeCapability)) {
        ...
        auto newWaitOp =
            builder.create<triton_rocm::gpu_rocm::AsyncWaitOp>(asyncWaitOp.getLoc(), 0);
  ...
  static Value promoteOperand(OpBuilder &builder, Location loc, Value operand,
    ...
    Type tensorPromotedType =
        operand.getType().cast<RankedTensorType>().cloneWith(std::nullopt,
                                                             promotedType);
    return builder.create<triton_rocm::FpToFpOp>(loc, tensorPromotedType, operand);
  }

  // promote operands of dot op if the existing combination is not natively
  // supported.
  void decomposeMixedModeDotOp(ModuleOp mod) const {
    mod.walk([](triton_rocm::DotOp dotOp) -> void {
...
std::unique_ptr<OperationPass<ModuleOp>> createConvertTritonGPUROCMToLLVMPass(
    ...
    mlir::triton_rocm::gpu_rocm::TMAMetadataTy *tmaMetadata) {
  return std::make_unique<ConvertTritonGPUROCMToLLVM>(computeCapability, target,
                                                  tmaMetadata);
}
