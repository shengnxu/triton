...
TritonGPUROCMToLLVMTypeConverter::TritonGPUROCMToLLVMTypeConverter(
    MLIRContext *ctx, LowerToLLVMOptions &option,
    const DataLayoutAnalysis *analysis)
    : LLVMTypeConverter(ctx, option, analysis) {
  addConversion([&](triton_rocm::PointerType type) -> std::optional<Type> {
...
Type TritonGPUROCMToLLVMTypeConverter::convertTritonPointerType(
    triton_rocm::PointerType type) {
...
Value TritonGPUROCMToLLVMTypeConverter::packLLElements(
...
SmallVector<Value> TritonGPUROCMToLLVMTypeConverter::packMfmaOperand(
...
SmallVector<Value> TritonGPUROCMToLLVMTypeConverter::unpackLLElements(
    Location loc, Value llvmStruct, ConversionPatternRewriter &rewriter,
    Type type) {
  assert(bool(llvmStruct) && "can not unpack null values");
  if (llvmStruct.getType().isIntOrIndexOrFloat() ||
      llvmStruct.getType().isa<triton_rocm::PointerType>() ||
...
Type TritonGPUROCMToLLVMTypeConverter::getElementTypeForStruct(
...
Type TritonGPUROCMToLLVMTypeConverter::convertTritonTensorType(
    RankedTensorType type) {
  auto ctx = type.getContext();
  Attribute layout = type.getEncoding();
