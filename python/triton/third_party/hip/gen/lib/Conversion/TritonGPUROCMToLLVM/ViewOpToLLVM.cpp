...
struct SplatOpConversion
    : public ConvertTritonGPUROCMOpToLLVMPattern<triton_rocm::SplatOp> {
  using ConvertTritonGPUROCMOpToLLVMPattern<
      triton_rocm::SplatOp>::ConvertTritonGPUROCMOpToLLVMPattern;
  ...
  LogicalResult matchAndRewrite(triton_rocm::SplatOp op, OpAdaptor adaptor,
...
struct ArithConstantSplatOpConversion
    : public ConvertTritonGPUROCMOpToLLVMPattern<arith::ConstantOp> {
  using ConvertTritonGPUROCMOpToLLVMPattern<
      arith::ConstantOp>::ConvertTritonGPUROCMOpToLLVMPattern;
...
struct CatOpConversion : public ConvertTritonGPUROCMOpToLLVMPattern<CatOp> {
  using OpAdaptor = typename CatOp::Adaptor;

  explicit CatOpConversion(TritonGPUROCMToLLVMTypeConverter &typeConverter,

                           PatternBenefit benefit = 1)
      : ConvertTritonGPUROCMOpToLLVMPattern<CatOp>(typeConverter, benefit) {}
...
struct ViewOpConversion : public ConvertTritonGPUROCMOpToLLVMPattern<ViewOp> {
  using OpAdaptor = typename ViewOp::Adaptor;
  explicit ViewOpConversion(TritonGPUROCMToLLVMTypeConverter &typeConverter,

                            PatternBenefit benefit = 1)
      : ConvertTritonGPUROCMOpToLLVMPattern<ViewOp>(typeConverter, benefit) {}

  LogicalResult
  matchAndRewrite(ViewOp op, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    Location loc = op->getLoc();
    assert(!triton_rocm::gpu_rocm::isExpensiveView(op.getSrc().getType(), op.getType()) &&
...
struct ExpandDimsOpConversion
    : public ConvertTritonGPUROCMOpToLLVMPattern<ExpandDimsOp> {
  using OpAdaptor = typename ExpandDimsOp::Adaptor;
  explicit ExpandDimsOpConversion(TritonGPUROCMToLLVMTypeConverter &typeConverter,

                                  PatternBenefit benefit = 1)
      : ConvertTritonGPUROCMOpToLLVMPattern<ExpandDimsOp>(typeConverter, benefit) {}
...
struct TransOpConversion
    : public ConvertTritonGPUROCMOpToLLVMPattern<triton_rocm::TransOp> {
  using ConvertTritonGPUROCMOpToLLVMPattern<
      triton_rocm::TransOp>::ConvertTritonGPUROCMOpToLLVMPattern;
