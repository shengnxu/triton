...
#include "triton/Dialect/TritonGPUROCM/IR/Dialect.cpp.inc"
#include "triton/Dialect/TritonGPUROCM/IR/Dialect.h"
#include "llvm/ADT/TypeSwitch.h"

using namespace mlir;
using namespace mlir::triton_rocm::gpu_rocm;
...
static Type getI1SameShapeFromTensorOrTensorPtr(Type type) {
  ...
  } else if (auto ptrType = type.dyn_cast<triton_rocm::PointerType>()) {
...
SmallVector<unsigned> getElemsPerThread(Type type) {
  if (type.isIntOrIndexOrFloat() || type.isa<triton_rocm::PointerType>())
...
bool isSharedEncoding(Value value) {
  ...
  if (auto tensorType = type.dyn_cast<RankedTensorType>()) {
    ...
    return encoding && encoding.isa<triton_rocm::gpu_rocm::SharedEncodingAttr>();
  }
  return false;
}
...
bool isExpensiveCat(CatOp cat, Attribute targetEncoding) {
  ...
  auto totalElemsPerThread = triton_rocm::gpu_rocm::getTotalElemsPerThread(tensorTy);
  auto shape = tensorTy.getShape();
  auto elemTy = tensorTy.getElementType();
  auto newTotalElemsPerThread =
      triton_rocm::gpu_rocm::getTotalElemsPerThread(targetEncoding, shape, elemTy);
...
#include "triton/Dialect/TritonGPUROCM/IR/TritonGPUAttrDefs.cpp.inc"
...
ParseResult parseInsertSliceOp(OpAsmParser &parser, OperationState &result) {
  ...
  if (allOperands.size() >= 4) {
    operandTypes.push_back(
        triton_rocm::getI1SameShapeFromTensorOrTensorPtr(srcType)); // mask
    hasMask = 1;
  }
  if (allOperands.size() >= 5) {
    operandTypes.push_back(triton_rocm::getPointeeType(srcType)); // other
    hasOther = 1;
  }
...
class TritonGPUROCMOpAsmInterface : public OpAsmDialectInterface {
public:
...
struct TritonGPUROCMInferLayoutInterface
    : public triton_rocm::DialectInferLayoutInterface {
  ...
  verifyDotOpEncodingCompatibility(Operation *op, Attribute operandEncodingA,
    ...
    auto aEncoding =
        operandEncodingA.dyn_cast<triton_rocm::gpu_rocm::DotOperandEncodingAttr>();
    auto bEncoding =
        operandEncodingB.dyn_cast<triton_rocm::gpu_rocm::DotOperandEncodingAttr>();
...
struct CanonicalizeConvertFromView
    : public mlir::OpRewritePattern<triton_rocm::ViewOp> {

  CanonicalizeConvertFromView(MLIRContext *context)
      : OpRewritePattern<triton_rocm::ViewOp>(context, 1) {}

  mlir::LogicalResult
  matchAndRewrite(triton_rocm::ViewOp op, PatternRewriter &rewriter) const override {
    ...
    if (isExpensiveView(convert.getOperand().getType(), op.getType()))
      return failure();
    // view(convert) -> view
    rewriter.replaceOpWithNewOp<triton_rocm::ViewOp>(op, op->getResult(0).getType(),
                                                convert.getOperand());
    return mlir::success();
  }
};
...
struct CanonicalizeConvertFromConvert
  ...
  matchAndRewrite(ConvertLayoutOp op,
    ...
    if (dstType.getEncoding().isa<triton_rocm::gpu_rocm::DotOperandEncodingAttr>() &&
        (srcType.getEncoding().isa<triton_rocm::gpu_rocm::MmaEncodingAttr>() ||
        srcType.getEncoding().isa<triton_rocm::gpu_rocm::MfmaEncodingAttr>()))
      return mlir::failure();
    // for hopper MMAv3
    if (!op.use_empty()) {
      bool hasDotUser = false;
      for (Operation *dot : op.getResult().getUsers())
        if (isa<triton_rocm::DotOp>(dot))
          hasDotUser = true;

      if (hasDotUser) {
        if (dstType.getEncoding().isa<triton_rocm::gpu_rocm::SharedEncodingAttr>() &&
            srcType.getEncoding().isa<triton_rocm::gpu_rocm::MmaEncodingAttr>())
          return mlir::failure();
      }
    }
    ...
    if (!arg)
      return mlir::failure();
    // cvt(view) -> view
    if (auto view = dyn_cast<triton_rocm::ViewOp>(arg)) {
      if (isExpensiveView(view.getOperand().getType(), op.getType()))
        return failure();
      rewriter.replaceOpWithNewOp<triton_rocm::ViewOp>(
          op, op->getResult(0).getType(), view.getResult());
      return mlir::success();
    }
    // cvt(cat) -> cat
    if (auto cat = dyn_cast<triton_rocm::CatOp>(arg)) {
      auto encoding =
          op->getResult(0).getType().cast<RankedTensorType>().getEncoding();
      if (isExpensiveCat(cat, encoding))
        return mlir::failure();
      rewriter.replaceOpWithNewOp<triton_rocm::CatOp>(op, op->getResult(0).getType(),
                                                 cat.getOperands());
      return mlir::success();
    }
    // cvt(alloc_tensor(x), type2) -> alloc_tensor(x, type2)
    auto alloc_tensor = dyn_cast<triton_rocm::gpu_rocm::AllocTensorOp>(arg);
    if (alloc_tensor) {
      if (!triton_rocm::gpu_rocm::isSharedEncoding(op->getResult(0))) {
        return mlir::failure();
      }
      rewriter.replaceOpWithNewOp<triton_rocm::gpu_rocm::AllocTensorOp>(
          op, op->getResult(0).getType());
      return mlir::success();
    }
    // cvt(insert_slice(x), type2) -> insert_slice(cvt(x, type2))
    auto insert_slice = dyn_cast<triton_rocm::gpu_rocm::InsertSliceAsyncOp>(arg);
    if (insert_slice) {
      if (!triton_rocm::gpu_rocm::isSharedEncoding(op->getResult(0))) {
        return mlir::failure();
      }
      ...
      auto newArg = rewriter.create<triton_rocm::gpu_rocm::ConvertLayoutOp>(
          op->getLoc(), newType, insert_slice.getDst());
      rewriter.replaceOpWithNewOp<triton_rocm::gpu_rocm::InsertSliceAsyncOp>(
    ...
    auto extract_slice = dyn_cast<triton_rocm::gpu_rocm::ExtractSliceOp>(arg);
    if (extract_slice) {
      if (!triton_rocm::gpu_rocm::isSharedEncoding(op->getResult(0))) {
        return mlir::failure();
      }
      ...
      rewriter.replaceOpWithNewOp<triton_rocm::gpu_rocm::ExtractSliceOp>(
    ...
    if (llvm::isa<triton_rocm::gpu_rocm::ConvertLayoutOp>(arg)) {
      if (arg->getOperand(0).getDefiningOp() &&
          !triton_rocm::gpu_rocm::isSharedEncoding(arg->getOperand(0)) &&
          triton_rocm::gpu_rocm::isSharedEncoding(op.getOperand()) &&
          !triton_rocm::gpu_rocm::isSharedEncoding(op.getResult())) {
        return mlir::failure();
      }
      if (triton_rocm::gpu_rocm::isSharedEncoding(op.getOperand()) &&
          triton_rocm::gpu_rocm::isSharedEncoding(op.getResult())) {
        return mlir::failure();
      }
      auto srcType = op.getOperand().getType().cast<RankedTensorType>();
      auto srcShared =
          srcType.getEncoding().dyn_cast<triton_rocm::gpu_rocm::SharedEncodingAttr>();
      if (srcShared && srcShared.getVec() > 1)
        return mlir::failure();
      rewriter.replaceOpWithNewOp<triton_rocm::gpu_rocm::ConvertLayoutOp>(
          op, op->getResultTypes().front(), arg->getOperand(0));
      return mlir::success();
    }
    // cvt(type1, splat(type2, x)) -> splat(type1, x)
    if (auto splat = llvm::dyn_cast<triton_rocm::SplatOp>(arg)) {
      rewriter.replaceOpWithNewOp<triton_rocm::SplatOp>(op, op->getResultTypes(),
                                                   splat.getSrc());
      return mlir::success();
    }
    // cvt(type1, make_range(type2, x)) -> make_range(type1, x)
    if (auto range = llvm::dyn_cast<triton_rocm::MakeRangeOp>(arg)) {
      rewriter.replaceOpWithNewOp<triton_rocm::MakeRangeOp>(
...
void TritonGPUROCMDialect::initialize() {
  registerTypes();
...
#include "triton/Dialect/TritonGPUROCM/IR/TritonGPUAttrDefs.cpp.inc"
  ...
  addOperations<
#define GET_OP_LIST
#include "triton/Dialect/TritonGPUROCM/IR/Ops.cpp.inc"
#include "triton/Dialect/TritonGPUROCM/IR/OpsEnums.cpp.inc"
      >();
  addInterfaces<TritonGPUROCMOpAsmInterface>();
  addInterfaces<TritonGPUROCMInferLayoutInterface>();
}
...
LogicalResult TritonGPUROCMDialect::verifyOperationAttribute(Operation *op,
                                                         NamedAttribute attr) {
  // TODO: fill this.
  return success();
}
