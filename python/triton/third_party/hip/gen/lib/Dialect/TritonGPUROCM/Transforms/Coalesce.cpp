...
static Value getMemAccessPtr(Operation *op) {
  if (auto ld = dyn_cast<triton_rocm::LoadOp>(op))
    return ld.getPtr();
  if (auto atomic = dyn_cast<triton_rocm::AtomicRMWOp>(op))
    return atomic.getPtr();
  if (auto atomic = dyn_cast<triton_rocm::AtomicCASOp>(op))
    return atomic.getPtr();
  if (auto insert = dyn_cast<triton_rocm::gpu_rocm::InsertSliceAsyncOp>(op))
    return insert.getSrc();
  if (auto store = dyn_cast<triton_rocm::StoreOp>(op))
    return store.getPtr();
  return nullptr;
}

struct CoalescePass : public TritonGPUROCMCoalesceBase<CoalescePass> {
  void
  setCoalescedEncoding(ModuleAxisInfoAnalysis &axisInfoAnalysis, Operation *op,
    ...
    auto queryAxisInfo = [&](const Value &val) -> AxisInfo {
      ...
      if (auto ptrType = valType.dyn_cast<PointerType>()) {
        ...
        auto tileShape = triton_rocm::gpu_rocm::getShapePerCTA(tensorTy);
    ...
    auto shapePerCTA = triton_rocm::gpu_rocm::getShapePerCTA(refTensorType);
    ...
    if (!dyn_cast<triton_rocm::LoadOp>(op)) {
    ...
    auto CTALayout = triton_rocm::gpu_rocm::getCTALayout(refTensorType.getEncoding());
    layoutMap[op] = triton_rocm::gpu_rocm::BlockedEncodingAttr::get(
  ...
  void coalesceOp(Attribute encoding, Operation *op) {
    ...
    for (auto operand : op->getOperands()) {
      ...
      if (tensorType &&
          !tensorType.getEncoding().isa<triton_rocm::gpu_rocm::SharedEncodingAttr>()) {
        Type newType = getNewType(tensorType, encoding);
        newArgs.push_back(builder.create<triton_rocm::gpu_rocm::ConvertLayoutOp>(
    ...
    for (auto t : op->getResultTypes()) {
      bool isAsync = isa<triton_rocm::gpu_rocm::InsertSliceAsyncOp>(op);
    ...
    for (size_t i = 0; i < op->getNumResults(); i++) {
      ...
      if (newTypes[i] != op->getResultTypes()[i]) {
        newResult = builder.create<triton_rocm::gpu_rocm::ConvertLayoutOp>(
