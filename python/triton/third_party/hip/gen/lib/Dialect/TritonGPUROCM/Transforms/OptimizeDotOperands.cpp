...
using triton_rocm::gpu_rocm::SharedEncodingAttr;
...
public:
  ...
  matchAndRewrite(mlir::Operation *op,
    ...
    auto tmpOp =
        dyn_cast_or_null<triton_rocm::TransOp>(dstOp.getSrc().getDefiningOp());
    if (!tmpOp)
      return mlir::failure();
    auto srcOp = dyn_cast_or_null<triton_rocm::gpu_rocm::ConvertLayoutOp>(
    ...
    auto XEncoding =
        XType.getEncoding().cast<triton_rocm::gpu_rocm::SharedEncodingAttr>();
    auto ZEncoding =
        ZType.getEncoding().dyn_cast<triton_rocm::gpu_rocm::DotOperandEncodingAttr>();
    if (!ZEncoding)
      return mlir::failure();
    // new X encoding
    ...
    auto newXOrder = triton_rocm::gpu_rocm::getOrder(argEncoding);
    ...
    auto newXEncoding = triton_rocm::gpu_rocm::SharedEncodingAttr::get(
    ...
    auto newX = rewriter.create<triton_rocm::gpu_rocm::ConvertLayoutOp>(srcOp.getLoc(),
                                                              newXType, arg);
    auto newY = rewriter.create<triton_rocm::TransOp>(tmpOp.getLoc(), newX);
    rewriter.replaceOpWithNewOp<triton_rocm::gpu_rocm::ConvertLayoutOp>(dstOp, ZType,
                                                              newY);
    return mlir::success();
  }
};
...
public:
  ...
  matchAndRewrite(mlir::Operation *op,
                  mlir::PatternRewriter &rewriter) const override {
    auto cvt = cast<triton_rocm::gpu_rocm::ConvertLayoutOp>(op);
    ...
    for (int i = 0; i < slice.size(); i++) {
      ...
      if (currOp->getParentRegion() != op->getParentRegion())
        continue;
      if (isa<triton_rocm::LoadOp>(currOp))
        checkOp = true;
      else if (checkOp) {
        if (!isa<triton_rocm::FpToFpOp, triton_rocm::BitcastOp>(currOp) &&
    ...
    if (!isa<triton_rocm::FpToFpOp, triton_rocm::BitcastOp>(cvtArgOp) &&
    ...
    if (!cvtTy.getEncoding().isa<triton_rocm::gpu_rocm::DotOperandEncodingAttr>())
      return mlir::failure();
    ...
    SmallVector<triton_rocm::gpu_rocm::ConvertLayoutOp> newCvts(numArgs);
    for (int i = 0; i < numArgs; i++)
      newCvts[i] = rewriter.create<triton_rocm::gpu_rocm::ConvertLayoutOp>(
...
public:
  ...
  matchAndRewrite(mlir::Operation *op,
    ...
    auto XEncoding =
        XType.getEncoding().cast<triton_rocm::gpu_rocm::SharedEncodingAttr>();
    auto ZEncoding =
        ZType.getEncoding().dyn_cast<triton_rocm::gpu_rocm::SharedEncodingAttr>();
    if (!ZEncoding)
      return mlir::failure();
    // new X encoding
    auto newXOrder = triton_rocm::gpu_rocm::getOrder(argEncoding);
    ...
    if (isa<triton_rocm::DotOp, triton_rocm::nvidia_gpu::DotAsyncOp>(dotOp)) {
      auto dotTy = dotOp->getResult(0).getType().cast<RankedTensorType>();
      auto dotEncoding =
          dotTy.getEncoding().dyn_cast<triton_rocm::gpu_rocm::MmaEncodingAttr>();
    ...
    auto newX = rewriter.create<triton_rocm::gpu_rocm::ConvertLayoutOp>(srcOp.getLoc(),
                                                              newXType, arg);
    rewriter.replaceOpWithNewOp<triton_rocm::TransOp>(dstOp, newX);
    return mlir::success();
  }
};

struct MMAV3UseRegOperand : public OpRewritePattern<triton_rocm::DotOp> {
  using OpRewritePattern<triton_rocm::DotOp>::OpRewritePattern;

  LogicalResult matchAndRewrite(triton_rocm::DotOp dotOp,
                                PatternRewriter &rewriter) const override {
    auto convertLhs =
        dotOp.getOperand(0).getDefiningOp<triton_rocm::gpu_rocm::ConvertLayoutOp>();
    if (!convertLhs)
      return failure();
...
class TritonGPUROCMOptimizeDotOperandsPass
    : public TritonGPUROCMOptimizeDotOperandsBase<
          TritonGPUROCMOptimizeDotOperandsPass> {
public:
  TritonGPUROCMOptimizeDotOperandsPass() = default;
  ...
  void runOnOperation() override {
    ...
    if (triton_rocm::gpu_rocm::TritonGPUROCMDialect::getComputeCapability(m) >= 80)
...
std::unique_ptr<Pass> mlir::createTritonGPUROCMOptimizeDotOperandsPass() {
  return std::make_unique<TritonGPUROCMOptimizeDotOperandsPass>();
}
