...
class TritonGPUROCMOptimizeThreadLocalityPass
    : public TritonGPUROCMOptimizeThreadLocalityBase<
          TritonGPUROCMOptimizeThreadLocalityPass> {
  void runOnOperation() override {
    ModuleOp mod = getOperation();
    DenseSet<triton_rocm::ReduceOp> reduceOps;
    mod.walk([&](triton_rocm::ReduceOp reduce) -> void {
      ...
      if (!(srcEncoding.isa<triton_rocm::gpu_rocm::BlockedEncodingAttr>() && rank > 1))
        return;
      for (auto operand : reduce->getOperands()) {
        auto def = operand.getDefiningOp();
        if (!isa<triton_rocm::LoadOp>(def))
          return;
      }
      auto elemsPerThread =
          triton_rocm::gpu_rocm::getElemsPerThread(srcType)[reduce.getAxis()];
    ...
    for (auto reduce : reduceOps) {
      ...
      auto srcEncoding = srcType.getEncoding();
      assert(srcEncoding.isa<triton_rocm::gpu_rocm::BlockedEncodingAttr>() &&
             "Thread locality optimization only supports blocked encoding");
      auto blocked = srcEncoding.dyn_cast<triton_rocm::gpu_rocm::BlockedEncodingAttr>();
      auto elemsPerThread =
          triton_rocm::gpu_rocm::getElemsPerThread(srcType)[reduce.getAxis()];
      ...
      auto slice2d = triton_rocm::gpu_rocm::SliceEncodingAttr::get(mod.getContext(), rank,
                                                         blocked3d);
      // Get forOp
...
private:
  std::optional<Operation *> getReductionOp(triton_rocm::ReduceOp reduce) const {
  ...
  Operation *createReduce(OpBuilder &builder, triton_rocm::ReduceOp reduce,
    ...
    IRMapping mapping;
    for (auto operand : reduce.getOperands()) {
      auto viewOp = builder.create<triton_rocm::ViewOp>(reduce.getLoc(),
  ...
  Operation *createAccum(OpBuilder &builder, triton_rocm::ReduceOp reduce,
  ...
  SmallVector<int64_t>
  getThreadLocalityOptimizedShape(triton_rocm::ReduceOp reduce) const {
    auto srcType = reduce.getOperands()[0].getType().cast<RankedTensorType>();
    auto srcShape = srcType.getShape();
    auto rank = srcShape.size();
    auto elemsPerThread =
  ...
  Attribute getThreadLocalityOptimizedEncoding(triton_rocm::ReduceOp reduce) const {
    auto srcType = reduce.getOperands()[0].getType().cast<RankedTensorType>();
    auto rank = srcType.getShape().size();
    auto srcEncoding = srcType.getEncoding();
    auto blocked = srcEncoding.dyn_cast<triton_rocm::gpu_rocm::BlockedEncodingAttr>();
    ...
    auto ctaLayout3d = triton_rocm::gpu_rocm::CTALayoutAttr::get(
        reduce.getContext(), ctasPerCGA3d, ctasSplitNum3d, ctaOrder3d);
    auto blocked3d = triton_rocm::gpu_rocm::BlockedEncodingAttr::get(
...
std::unique_ptr<Pass> mlir::createTritonGPUROCMOptimizeThreadLocalityPass() {
  return std::make_unique<TritonGPUROCMOptimizeThreadLocalityPass>();
}
