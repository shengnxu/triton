...
protected:
  ...
  Value step;
  bool dynamicLoop;
  triton_rocm::PipeliningOption::AnnotationlFnType annotateFn = nullptr;
  bool peelEpilogue;
  triton_rocm::PipeliningOption::PredicateOpFnType predicateFn = nullptr;
...
public:
  ...
  bool initializeLoopInfo(ForOp op, const triton_rocm::PipeliningOption &options);
...
bool LoopPipelinerInternal::initializeLoopInfo(
    ForOp op, const triton_rocm::PipeliningOption &options) {
...
void LoopPipelinerInternal::emitPrologue(RewriterBase &rewriter) {
  ...
  for (int64_t i = 0; i < maxStage; i++) {
    ...
    for (Operation *op : opOrder) {
      ...
      if (annotateFn)
        annotateFn(newOp, triton_rocm::PipeliningOption::PipelinerPart::Prologue, i);
...
LogicalResult LoopPipelinerInternal::createKernel(
  ...
  for (Operation *op : opOrder) {
    ...
    rewriter.setInsertionPointAfter(newOp);
    if (annotateFn)
      annotateFn(newOp, triton_rocm::PipeliningOption::PipelinerPart::Kernel, 0);
  }
...
LoopPipelinerInternal::emitEpilogue(RewriterBase &rewriter) {
  ...
  for (int64_t i = 1; i <= maxStage; i++) {
    for (Operation *op : opOrder) {
      ...
      if (annotateFn)
        annotateFn(newOp, triton_rocm::PipeliningOption::PipelinerPart::Epilogue,
                   i - 1);
      for (unsigned destId : llvm::seq(unsigned(0), op->getNumResults())) {
        setValueMapping(op->getResult(destId), newOp->getResult(destId),
...
FailureOr<ForOp>
mlir::triton_rocm::pipelineForLoop(RewriterBase &rewriter, ForOp forOp,
                              const triton_rocm::PipeliningOption &options,
                              bool *modifiedIR) {
  if (modifiedIR)
    *modifiedIR = false;
