...
Value Prefetcher::generatePrefetch(Value v, unsigned opIdx, bool isPrologue,
  ...
  Value newSmem = builder.create<triton_rocm::gpu_rocm::ExtractSliceOp>(
  ...
  auto dotOperandEnc = triton_rocm::gpu_rocm::DotOperandEncodingAttr::get(
      builder.getContext(), opIdx, dotEncoding, prefetchWidth / 8);
  Value prefetchSlice = builder.create<triton_rocm::gpu_rocm::ConvertLayoutOp>(
...
LogicalResult Prefetcher::initialize() {
  ...
  SmallVector<triton_rocm::DotOp> dotsInFor;
  for (Operation &op : *loop)
    if (auto dotOp = dyn_cast<triton_rocm::DotOp>(op))
  ...
  auto getPrefetchSrc = [](Value v) -> SmallVector<Value> {
    ...
    while (op) {
      ...
      if (auto cvt = dyn_cast_or_null<triton_rocm::gpu_rocm::ConvertLayoutOp>(op))
        if (triton_rocm::gpu_rocm::isSharedEncoding(cvt.getOperand())) {
  ...
  for (triton_rocm::DotOp dot : dotsInFor) {
    auto aType = dot.getA().getType().cast<RankedTensorType>();
    auto bType = dot.getB().getType().cast<RankedTensorType>();
    auto aEnc = aType.getEncoding().cast<triton_rocm::gpu_rocm::DotOperandEncodingAttr>();
    auto bEnc = bType.getEncoding().cast<triton_rocm::gpu_rocm::DotOperandEncodingAttr>();
...
void Prefetcher::emitPrologue() {
  ...
  for (Value dot : dots) {
    ...
    operand2headPrefetch[dot.getDefiningOp<triton_rocm::DotOp>().getA()] =
        aPrefetched;
    operand2headPrefetch[dot.getDefiningOp<triton_rocm::DotOp>().getB()] =
        bPrefetched;
  }
}
...
scf::ForOp Prefetcher::createNewForOp() {
  ...
  for (Operation &op : forOp.getBody()->without_terminator()) {
    ...
    auto dot = dyn_cast<triton_rocm::DotOp>(&op);
...
struct PrefetchPass : public TritonGPUROCMPrefetchBase<PrefetchPass> {
