...
namespace {
using triton_rocm::DotOp;
using triton_rocm::gpu_rocm::ConvertLayoutOp;
using triton_rocm::gpu_rocm::DotOperandEncodingAttr;
using triton_rocm::gpu_rocm::MmaEncodingAttr;
using triton_rocm::gpu_rocm::SliceEncodingAttr;
...
public:
  ConvertDotConvert(mlir::MLIRContext *context)
      : mlir::RewritePattern(triton_rocm::gpu_rocm::ConvertLayoutOp::getOperationName(),
                             1, context) {}

  LogicalResult
  matchAndRewrite(mlir::Operation *op,
                  mlir::PatternRewriter &rewriter) const override {
    auto dstOp = cast<triton_rocm::gpu_rocm::ConvertLayoutOp>(op);
    auto dotOp = dstOp.getSrc().getDefiningOp<triton_rocm::DotOp>();
    if (!dotOp)
      return mlir::failure();
    if (std::distance(dstOp->user_begin(), dstOp->user_end()) != 1 ||
      ...
      return mlir::failure();
    auto cvtOp =
        dotOp.getOperand(2).getDefiningOp<triton_rocm::gpu_rocm::ConvertLayoutOp>();
    if (!cvtOp)
      return mlir::failure();
    if (!cvtOp.getSrc().getDefiningOp<triton_rocm::LoadOp>())
      return failure();
    ...
    auto _0 = rewriter.create<triton_rocm::SplatOp>(
        op->getLoc(), dotOp.getResult().getType(), _0f);
    auto newDot = rewriter.create<triton_rocm::DotOp>(
        op->getLoc(), dotOp.getResult().getType(), dotOp.getOperand(0),
        dotOp.getOperand(1), _0, dotOp.getAllowTF32(),
        dotOp.getMaxNumImpreciseAcc());
    auto newCvt = rewriter.create<triton_rocm::gpu_rocm::ConvertLayoutOp>(
...
public:
  ...
  LayoutPropagation(triton_rocm::FuncOp F) : funcOp(F) {}
...
static bool hasConvertToMMATransisitiveUse(Operation *op, Attribute encoding) {
  ...
  while (!queue.empty()) {
    ...
    for (Operation *op : forwardSlice) {
      if (auto convertOp = dyn_cast<triton_rocm::gpu_rocm::ConvertLayoutOp>(op)) {
        Attribute dstEncoding = convertOp.getResult()
                                    .getType()
                                    .cast<RankedTensorType>()
                                    .getEncoding();
        if (auto mmaLayout =
                dstEncoding.dyn_cast<triton_rocm::gpu_rocm::MmaEncodingAttr>())
          return (mmaLayout.getVersionMajor() > 1) ? true
                                                   : mmaLayout == encoding;
        if (dstEncoding.isa<triton_rocm::gpu_rocm::DotOperandEncodingAttr>())
          return encoding.cast<triton_rocm::gpu_rocm::MmaEncodingAttr>()
                     .getVersionMajor() > 1;
      }
...
static bool isLayoutAnchor(Operation *op) {
  if (isa<triton_rocm::LoadOp, triton_rocm::StoreOp>(op))
    return isExpensiveLoadOrStore(op);
  if (isa<triton_rocm::ViewOp, triton_rocm::DotOp, triton_rocm::AtomicRMWOp,
          triton_rocm::AtomicCASOp>(op))
    return true;
  return false;
}
    ...
    if (isLayoutAnchor(op)) {
      for (auto result : op->getResults()) {
        if (auto tensorType = result.getType().dyn_cast<RankedTensorType>()) {
          ...
          if (tensorType.getEncoding().isa<triton_rocm::gpu_rocm::MmaEncodingAttr>() &&
          ...
          // into global memory.
          if (tensorType.getEncoding().isa<triton_rocm::gpu_rocm::MfmaEncodingAttr>() &&
...
SmallVector<Value> LayoutPropagation::propagateToUsers(Value value,
  ...
  for (OpOperand &use : value.getUses()) {
    ...
    if (user->hasTrait<mlir::OpTrait::SameOperandsAndResultEncodingROCM>() ||
        user->hasTrait<mlir::OpTrait::Elementwise>() ||
        isa<triton_rocm::ReduceOp, triton_rocm::ExpandDimsOp,
            triton_rocm::gpu_rocm::ConvertLayoutOp>(user)) {
#if 1
      if (auto convertOp = dyn_cast<triton_rocm::gpu_rocm::ConvertLayoutOp>(user)) {
        if (triton_rocm::gpu_rocm::isSharedEncoding(convertOp.getResult()) ||
            triton_rocm::gpu_rocm::isSharedEncoding(convertOp.getOperand()))
          continue;
      }
#endif
...
void LayoutPropagation::resolveConflicts() {
  for (auto &it : layouts) {
    ...
    bool isLoadOrStore =
        op && isa<triton_rocm::LoadOp, triton_rocm::StoreOp, triton_rocm::AtomicRMWOp,
                  triton_rocm::AtomicCASOp>(op);
    for (Attribute e : info.encodings) {
      if ((isLoadOrStore && e.isa<triton_rocm::gpu_rocm::BlockedEncodingAttr>()) ||
          (!isLoadOrStore && e.isa<triton_rocm::gpu_rocm::MmaEncodingAttr>())) {
        encoding = e;
        break;
      }
    }
...
static bool reduceToScalar(Operation *op) {
  ...
  return isa<triton_rocm::ReduceOp>(op) &&
...
Value LayoutPropagation::getValueAs(Value value, Attribute encoding) {
  if (auto tensorType = value.getType().dyn_cast<RankedTensorType>()) {
    ...
    if (Operation *op = rewrittenValue.getDefiningOp()) {
      if (isa<triton_rocm::gpu_rocm::AsyncWaitOp>(op->getNextNode()))
        rewriter.setInsertionPointAfter(op->getNextNode());
    }
    auto tmpType = RankedTensorType::get(tensorType.getShape(),
                                         tensorType.getElementType(), encoding);
    Value converted = rewriter.create<triton_rocm::gpu_rocm::ConvertLayoutOp>(
...
Operation *LayoutPropagation::rewriteOp(Operation *op) {
  ...
  if (auto convertOp = dyn_cast<triton_rocm::gpu_rocm::ConvertLayoutOp>(op)) {
    ...
    auto newType = RankedTensorType::get(tensorType.getShape(),
                                         tensorType.getElementType(), encoding);
    auto cvt = rewriter.create<triton_rocm::gpu_rocm::ConvertLayoutOp>(op->getLoc(),
                                                             newType, src);
  ...
  if (canFoldIntoConversion(op, encoding)) {
    ...
    auto newType = RankedTensorType::get(tensorType.getShape(),
                                         tensorType.getElementType(), encoding);
    auto cvt = rewriter.create<triton_rocm::gpu_rocm::ConvertLayoutOp>(
        op->getLoc(), newType, newOp->getResult(0));
    map(op->getResult(0), cvt.getResult());
    return cvt.getOperation();
  }
  if (op->hasTrait<mlir::OpTrait::SameOperandsAndResultEncodingROCM>() ||
      op->hasTrait<mlir::OpTrait::Elementwise>() ||
      isa<triton_rocm::ReduceOp, triton_rocm::ExpandDimsOp, triton_rocm::gpu_rocm::ConvertLayoutOp>(
          op)) {
...
static bool canBeRemat(Operation *op) {
  if (isa<triton_rocm::LoadOp, triton_rocm::StoreOp>(op))
    return !isExpensiveLoadOrStore(op);
  if (isa<tensor::ExtractSliceOp, triton_rocm::gpu_rocm::AllocTensorOp,
          triton_rocm::gpu_rocm::InsertSliceAsyncOp, triton_rocm::AtomicRMWOp,
          triton_rocm::AtomicCASOp, triton_rocm::DotOp>(op))
    return false;
...
static void rewriteSlice(SetVector<Value> &slice,
  ...
  for (Operation *op : opsToRewrite) {
    ...
    if (isa<arith::ConstantOp>(op)) {
      ...
      auto cvt = builder.create<triton_rocm::gpu_rocm::ConvertLayoutOp>(
          op->getLoc(), newType, newOp->getResult(0));
...
static void backwardRematerialization(ConvertLayoutOp convertOp) {
  ...
  if (targetType.getEncoding().isa<triton_rocm::gpu_rocm::DotOperandEncodingAttr>())
    return;
...
static void hoistConvertOnTopOfExtOrBroadcast(ConvertLayoutOp convertOp) {
  ...
  auto isExtOrBroadcastOp = [](Operation *op) {
    return isa<arith::ExtSIOp, arith::ExtUIOp, arith::ExtFOp,
               triton_rocm::BroadcastOp, triton_rocm::ExpandDimsOp>(op);
  };
...
class TritonGPUROCMRemoveLayoutConversionsPass
    : public TritonGPUROCMRemoveLayoutConversionsBase<
          TritonGPUROCMRemoveLayoutConversionsPass> {
public:
  TritonGPUROCMRemoveLayoutConversionsPass() = default;
  ...
  void runOnOperation() override {
    ...
    m.walk([](triton_rocm::FuncOp funcOp) {
...
std::unique_ptr<Pass> mlir::createTritonGPUROCMRemoveLayoutConversionsPass() {
  return std::make_unique<TritonGPUROCMRemoveLayoutConversionsPass>();
}
