...
static inline bool
willIncreaseRegisterPressure(triton_rocm::gpu_rocm::ConvertLayoutOp op) {
...
class TritonGPUROCMReorderInstructionsPass
    : public TritonGPUROCMReorderInstructionsBase<
          TritonGPUROCMReorderInstructionsPass> {
public:
  TritonGPUROCMReorderInstructionsPass() = default;
  ...
  void runOnOperation() override {
    ...
    m.walk([&](triton_rocm::gpu_rocm::ConvertLayoutOp op) {
...
#if 1
      if (!dstEncoding.isa<triton_rocm::gpu_rocm::SharedEncodingAttr>() &&
          !dstEncoding.isa<triton_rocm::gpu_rocm::DotOperandEncodingAttr>())
        return;
#elif
    ...
    m.walk([&](triton_rocm::gpu_rocm::ConvertLayoutOp op) {
      ...
      if (op->getUsers().empty())
        return;
      auto dotUser = dyn_cast<triton_rocm::DotOp>(*op->user_begin());
      if (!dotUser)
        return;
      auto AOp =
          dotUser.getOperand(0).getDefiningOp<triton_rocm::gpu_rocm::ConvertLayoutOp>();
      if (!AOp)
        return;
...
std::unique_ptr<Pass> mlir::createTritonGPUROCMReorderInstructionsPass() {
  return std::make_unique<TritonGPUROCMReorderInstructionsPass>();
}
