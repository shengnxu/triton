...
class LoopPipeliner {
  ...
  Value getLoadMask(triton_rocm::LoadOp loadOp, Value mappedMask,
  ...
  void updateLoadMask(triton_rocm::LoadOp loadOp, Value newMask);
...
LogicalResult LoopPipeliner::checkOpUses() {
  ...
  for (Operation *op : ops) {
    ...
    for (Operation *other : ops)
      if (isa<triton_rocm::LoadOp>(other))
...
Value LoopPipeliner::getLoadMask(triton_rocm::LoadOp loadOp, Value mappedMask,
                                 Value loopCond, OpBuilder &builder) {
  if (!peelLastIter) {
    // add mask for last iteration when not peeled to epilogue
    Value mask = loadOp.getMask();
    Type maskType = triton_rocm::getI1SameShape(loadOp.getType());
    Value newMask;
    if (mask) {
      Value cond = loopCond;
      if (isa<RankedTensorType>(maskType)) {
        cond = builder.create<triton_rocm::SplatOp>(mask.getLoc(), maskType, loopCond);
      }
      newMask = builder.create<arith::AndIOp>(mask.getLoc(), mappedMask, cond);
    } else {
      if (isa<RankedTensorType>(maskType)) {
        newMask = builder.create<triton_rocm::SplatOp>(loopCond.getLoc(), maskType,
                                                  loopCond);
      } else {
        newMask = loopCond;
      }
...
bool LoopPipeliner::isLoadChain(Operation *op) const {
  if (auto cvtOp = dyn_cast<triton_rocm::gpu_rocm::ConvertLayoutOp>(op)) {
    Value loadVal = cvtOp.getSrc();
    if (auto f2fOp = dyn_cast<triton_rocm::FpToFpOp>(op))
...
void LoopPipeliner::emitPrologue() {
  ...
  for (Operation *op : orderedDeps) {
    ...
    if (validLoads.contains(op)) {
      auto loadOp = cast<triton_rocm::LoadOp>(op);
...
void LoopPipeliner::updateLoadMask(triton_rocm::LoadOp loadOp, Value newMask) {
  if (newMask) {
...
void LoopPipeliner::prefetchNextBuffer(OpBuilder &builder) {
  ...
  for (Operation *op : orderedDeps) {
    ...
    if (validLoads.contains(op)) {
      // Update loading mask
      auto loadOp = llvm::cast<triton_rocm::LoadOp>(op);
      ...
      updateLoadMask(cast<triton_rocm::LoadOp>(newOp), newMask);
      ...
      if (auto loadOp = dyn_cast<triton_rocm::LoadOp>(op)) {
        if (auto newMask = getLoadMask(loadOp,
                                       ...
                                       nextLoopCond, builder)) {
          updateLoadMask(cast<triton_rocm::LoadOp>(nextOp), newMask);
        }
      }
...
// Stream Pipeline
struct PipelinePass : public TritonGPUROCMStreamPipelineBase<PipelinePass> {
