...
// TypeConverter
//
TritonGPUROCMTypeConverter::TritonGPUROCMTypeConverter(MLIRContext *context,
  ...
  addConversion([this](RankedTensorType tensorType) -> RankedTensorType {
    ...
    Attribute encoding = triton_rocm::gpu_rocm::BlockedEncodingAttr::get(
  ...
  addConversion([this](triton_rocm::PointerType ptrType) -> triton_rocm::PointerType {
    ...
    return triton_rocm::PointerType::get(convertedTensorType,
  ...
  addTargetMaterialization([&](OpBuilder &builder, RankedTensorType tensorType,
    ...
    auto cast =
        builder.create<triton_rocm::gpu_rocm::ConvertLayoutOp>(loc, tensorType, inputs);
...
// TritonGPUConversion
//
TritonGPUROCMConversionTarget::TritonGPUROCMConversionTarget(
    MLIRContext &context, TritonGPUROCMTypeConverter &typeConverter)
    : ConversionTarget(context) {
  // TODO: we should also verify ops of TritonGPUDialect
  addLegalDialect<triton_rocm::gpu_rocm::TritonGPUROCMDialect>();
  ...
  addDynamicallyLegalDialect<arith::ArithDialect, math::MathDialect,
                             triton_rocm::TritonROCMDialect, cf::ControlFlowDialect,
  ...
  addDynamicallyLegalOp<triton_rocm::DotOp>([](triton_rocm::DotOp dotOp) -> bool {
    Attribute aEncoding =
        dotOp.getA().getType().cast<RankedTensorType>().getEncoding();
    Attribute bEncoding =
        dotOp.getB().getType().cast<RankedTensorType>().getEncoding();
    if (aEncoding && aEncoding.isa<triton_rocm::gpu_rocm::DotOperandEncodingAttr>() &&
        bEncoding && bEncoding.isa<triton_rocm::gpu_rocm::DotOperandEncodingAttr>())
      return true;
    return false;
  });
}
