...
bool isLoadFromTensorPtr(triton_rocm::LoadOp op) {
  return mlir::triton_rocm::isTensorPointerType(op.getPtr().getType());
}
...
triton_rocm::gpu_rocm::SharedEncodingAttr getSharedEncoding(RankedTensorType tensorTy) {
  auto blockedLayout =
      tensorTy.getEncoding().cast<triton_rocm::gpu_rocm::BlockedEncodingAttr>();
  return triton_rocm::gpu_rocm::SharedEncodingAttr::get(
...
std::string GraphDumper::dump(triton_rocm::FuncOp func) const {
...
void GraphDumper::dumpToFile(triton_rocm::FuncOp func,
...
std::string GraphLayoutMarker::getColor(const Type &type) const {
  if (auto tensorTy = type.dyn_cast<RankedTensorType>()) {
    ...
    if (layout.isa<triton_rocm::gpu_rocm::BlockedEncodingAttr>())
      return "green";
    else if (layout.isa<triton_rocm::gpu_rocm::SliceEncodingAttr>())
      return "yellow";
    else if (layout.isa<triton_rocm::gpu_rocm::MmaEncodingAttr>())
      return "lightslateblue";
    else if (layout.isa<triton_rocm::gpu_rocm::DotOperandEncodingAttr>())
      return "orange";
    else if (layout.isa<triton_rocm::gpu_rocm::SharedEncodingAttr>())
      return "orangered";
    else {
...
static std::optional<Attribute> inferDstEncoding(triton_rocm::ReduceOp op,
                                                 Attribute encoding) {
  return triton_rocm::gpu_rocm::SliceEncodingAttr::get(op->getContext(), op.getAxis(),
                                             encoding);
}

static std::optional<Attribute> inferDstEncoding(triton_rocm::ExpandDimsOp op,
                                                 Attribute encoding) {
  auto sliceEncoding = encoding.dyn_cast<triton_rocm::gpu_rocm::SliceEncodingAttr>();
  if (!sliceEncoding)
    return std::nullopt;
...
static std::optional<Attribute> inferSrcEncoding(triton_rocm::ReduceOp op,
                                                 Attribute encoding) {
  auto sliceEncoding = encoding.dyn_cast<triton_rocm::gpu_rocm::SliceEncodingAttr>();
  if (!sliceEncoding)
    return std::nullopt;
...
static std::optional<Attribute> inferSrcEncoding(triton_rocm::ExpandDimsOp op,
                                                 Attribute encoding) {
  return triton_rocm::gpu_rocm::SliceEncodingAttr::get(op->getContext(), op.getAxis(),
                                             encoding);
}
...
std::optional<Attribute> inferSrcEncoding(Operation *op, Attribute encoding) {
  ...
  if (auto expand = dyn_cast<triton_rocm::ExpandDimsOp>(op))
    return inferSrcEncoding(expand, encoding);
  if (isa<triton_rocm::ViewOp, triton_rocm::CatOp>(op))
    return std::nullopt;
  return encoding;
}
...
bool isExpensiveLoadOrStore(Operation *op) {
  ...
  if (triton_rocm::isTensorPointerType(operandType))
    return true;
...
bool isExpensiveToRemat(Operation *op, Attribute &targetEncoding) {
  ...
  if (isa<triton_rocm::CatOp>(op))
    return triton_rocm::gpu_rocm::isExpensiveCat(cast<triton_rocm::CatOp>(op), targetEncoding);
...
bool canFoldIntoConversion(Operation *op, Attribute targetEncoding) {
  if (isa<triton_rocm::CatOp>(op))
    return !triton_rocm::gpu_rocm::isExpensiveCat(cast<triton_rocm::CatOp>(op),
                                        targetEncoding);
  if (auto convert = dyn_cast<triton_rocm::gpu_rocm::ConvertLayoutOp>(op)) {
    if (targetEncoding.isa<triton_rocm::gpu_rocm::MmaEncodingAttr>()) {
      auto srcEncoding =
  ...
  if (auto view = dyn_cast<triton_rocm::ViewOp>(op)) {
    ...
    return !triton_rocm::gpu_rocm::isExpensiveView(view.getOperand().getType(),
                                         newDstType);
  }
  return isa<triton_rocm::gpu_rocm::ConvertLayoutOp, arith::ConstantOp,
             triton_rocm::MakeRangeOp, triton_rocm::SplatOp>(op);
}
  ...
  while (!queue.empty()) {
    ...
    if (auto *definingOp = currentValue.getDefiningOp()) {
      ...
      if (stopPropagation && stopPropagation(definingOp))
        continue;
      if (isa<triton_rocm::CatOp>(definingOp))
        return failure();
