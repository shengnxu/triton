...
#define KEY_TYPES                                                              \
  triton_rocm::DotOp, triton_rocm::nvidia_gpu::DotAsyncOp, ttg::InsertSliceOp,           \
      triton_rocm::StoreOp
    ...
    if (isa<scf::ForOp>(lockOp)) {
      ...
      for (auto v : lockOp->getResults()) {
        ...
        auto iter = llvm::find_if(slices, [](Operation *op) {
          return isa<triton_rocm::nvidia_gpu::ConsumerReleaseOp>(op);
        });
        if (iter != slices.end()) {
...
struct WSMutexPass : public TritonGPUROCMWSMutexBase<WSMutexPass> {
public:
  WSMutexPass() = default;
...
std::unique_ptr<Pass>
mlir::createTritonNvidiaGPUROCMWSMutexPass(int computeCapability) {
