...
#include "triton/Dialect/TritonROCM/IR/Ops.cpp.inc"

// enum attribute definitions
#include "triton/Dialect/TritonROCM/IR/OpsEnums.cpp.inc"
...
void LoadOp::build(::mlir::OpBuilder &builder, ::mlir::OperationState &state,
                   ::mlir::Value ptr, ::mlir::triton_rocm::CacheModifier cache,
                   ::mlir::triton_rocm::EvictionPolicy evict, bool isVolatile) {
...
void LoadOp::build(::mlir::OpBuilder &builder, ::mlir::OperationState &state,
                   ...
                   std::optional<::mlir::triton_rocm::PaddingOption> padding,
                   ::mlir::triton_rocm::CacheModifier cache,
                   ::mlir::triton_rocm::EvictionPolicy evict, bool isVolatile) {
...
void LoadOp::build(::mlir::OpBuilder &builder, ::mlir::OperationState &state,
  ...
  if (padding.has_value()) {
    state.addAttribute(getPaddingAttrName(state.name),
                       ::mlir::triton_rocm::PaddingOptionAttr::get(
                           builder.getContext(), padding.value()));
  }
  state.addAttribute(
      getCacheAttrName(state.name),
      ::mlir::triton_rocm::CacheModifierAttr::get(builder.getContext(), cache));
  state.addAttribute(
      getEvictAttrName(state.name),
      ::mlir::triton_rocm::EvictionPolicyAttr::get(builder.getContext(), evict));
...
struct CanonicalizeMaskedLoadPattern
    : public mlir::OpRewritePattern<triton_rocm::LoadOp> {
  CanonicalizeMaskedLoadPattern(mlir::MLIRContext *context)
      : OpRewritePattern<triton_rocm::LoadOp>(context, 1) {}

  mlir::LogicalResult
  matchAndRewrite(triton_rocm::LoadOp loadOp,
    ...
    if (splatMask.getSplatValue<IntegerAttr>().getValue() == true) {
      // mask = splat(1)
      rewriter.replaceOpWithNewOp<triton_rocm::LoadOp>(
...
void triton_rocm::LoadOp::getCanonicalizationPatterns(RewritePatternSet &results,
                                                 MLIRContext *context) {
...
void StoreOp::build(::mlir::OpBuilder &builder, ::mlir::OperationState &state,
                    ...
                    ::mlir::triton_rocm::CacheModifier cache,
                    ::mlir::triton_rocm::EvictionPolicy evict) {
...
struct CanonicalizeMaskedStorePattern
    : public mlir::OpRewritePattern<triton_rocm::StoreOp> {
  CanonicalizeMaskedStorePattern(mlir::MLIRContext *context)
      : OpRewritePattern<triton_rocm::StoreOp>(context, 1) {}

  mlir::LogicalResult
  matchAndRewrite(triton_rocm::StoreOp storeOp,
                  mlir::PatternRewriter &rewriter) const override {
...
void triton_rocm::StoreOp::getCanonicalizationPatterns(RewritePatternSet &results,
                                                  MLIRContext *context) {
  results.add<CanonicalizeMaskedStorePattern>(context);
}

//-- TransOp --
mlir::LogicalResult mlir::triton_rocm::TransOp::inferReturnTypes(
...
//-- DotOp --
mlir::LogicalResult mlir::triton_rocm::DotOp::inferReturnTypes(
    MLIRContext *context, std::optional<Location> location, ValueRange operands,
    DictionaryAttr attributes, OpaqueProperties properties, RegionRange regions,
    SmallVectorImpl<Type> &inferredReturnTypes) {
...
LogicalResult mlir::triton_rocm::DotOp::verify() {
...
mlir::LogicalResult mlir::triton_rocm::ReduceOp::inferReturnTypes(
    MLIRContext *context, std::optional<Location> location, ValueRange operands,
    DictionaryAttr attributes, OpaqueProperties properties, RegionRange regions,
    SmallVectorImpl<Type> &inferredReturnTypes) {
...
mlir::LogicalResult mlir::triton_rocm::ReduceOp::verify() {
...
mlir::LogicalResult mlir::triton_rocm::ReduceOp::verifyRegions() {
  ...
  auto terminator =
      dyn_cast<mlir::triton_rocm::ReduceReturnOp>(block.getTerminator());
  if (!terminator) {
    return this->emitOpError()
...
mlir::LogicalResult mlir::triton_rocm::ScanOp::inferReturnTypes(
    MLIRContext *context, std::optional<Location> location, ValueRange operands,
    DictionaryAttr attributes, OpaqueProperties properties, RegionRange regions,
    SmallVectorImpl<Type> &inferredReturnTypes) {
  for (auto arg : operands)
...
mlir::LogicalResult mlir::triton_rocm::ScanOp::verify() {
  if (this->getOperands().size() < 1) {
    return this->emitOpError() << "must have at least 1 operand";
  }
  for (const auto &operand : this->getOperands()) {
...
//-- ExpandDimsOp --
mlir::LogicalResult mlir::triton_rocm::ExpandDimsOp::inferReturnTypes(
...
LogicalResult ExpandDimsOp::canonicalize(ExpandDimsOp op,
  ...
  if (auto splat = dyn_cast<triton_rocm::SplatOp>(definingOp)) {
    rewriter.replaceOpWithNewOp<triton_rocm::SplatOp>(op, op.getType(),
                                                 splat.getOperand());
    return mlir::success();
  }
  ...
  if (auto broadcast = dyn_cast<triton_rocm::BroadcastOp>(definingOp)) {
    ...
    auto newExpand = rewriter.create<triton_rocm::ExpandDimsOp>(
        op.getLoc(), newExpandTy, src, op.getAxis());
    auto newBroadcast = rewriter.create<triton_rocm::BroadcastOp>(
...
//-- FuncOp --
void triton_rocm::FuncOp::build(OpBuilder &builder, OperationState &state,
...
ParseResult triton_rocm::FuncOp::parse(OpAsmParser &parser, OperationState &result) {
  auto buildFuncType =
...
void triton_rocm::FuncOp::print(OpAsmPrinter &printer) {
...
// -- CallOp --
LogicalResult
triton_rocm::CallOp::verifySymbolUses(mlir::SymbolTableCollection &symbolTable) {
...
// -- ReturnOp --
LogicalResult triton_rocm::ReturnOp::verify() {
  auto function = cast<triton_rocm::FuncOp>((*this)->getParentOp());
