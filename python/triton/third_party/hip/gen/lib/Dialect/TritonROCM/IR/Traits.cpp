#include "triton/Dialect/TritonROCM/IR/Traits.h"
...
static LogicalResult verifySameEncoding(Type typeA, Type typeB,
  ...
  auto getEncoding = [=](Type type) -> Attribute {
    ...
    if (!allowTensorPointerType) {
      assert(!triton_rocm::isTensorPointerType(type));
    }
    return ret;
  };
...
LogicalResult
OpTrait::impl::verifySameOperandsEncodingROCM(Operation *op,
                                          bool allowTensorPointerType) {
...
LogicalResult OpTrait::impl::verifySameOperandsAndResultEncodingROCM(
  ...
  return verifySameOperandsEncodingROCM(op, allowTensorPointerType);
}
  ...
  for (auto opType : op->getOperandTypes()) {
    if (auto tensorType = opType.dyn_cast<RankedTensorType>()) {
      ...
      for (int64_t s : tensorType.getShape())
        numElements *= s;
      if (numElements > maxTensorNumElementsROCM)
        return op->emitError("Maximum allowed number of elements is ")
               << maxTensorNumElementsROCM << ", but " << *op
               << " has more than that";
...
static ArrayRef<int64_t> getTypeShape(Type type) {
  ...
  if (auto ptrType = type.dyn_cast<triton_rocm::PointerType>())
    rankedType = ptrType.getPointeeType().dyn_cast<RankedTensorType>();
  return rankedType ? rankedType.getShape() : ArrayRef<int64_t>();
}

LogicalResult OpTrait::impl::verifySameLoadStoreOperandsShapeROCM(Operation *op) {
  if (failed(verifyAtLeastNOperands(op, 1)))
    return failure();
...
LogicalResult
OpTrait::impl::verifySameLoadStoreOperandsAndResultShapeROCM(Operation *op) {
  if (failed(verifyAtLeastNOperands(op, 1)) ||
      failed(verifyAtLeastNResults(op, 1)))
    return failure();
...
bool OpTrait::impl::verifyLoadStorePointerAndValueType(Type valueType,
  ...
  if (triton_rocm::isTensorPointerType(ptrType)) {
    ...
    if (auto elementPtrType =
            dyn_cast<triton_rocm::PointerType>(rankedType.getElementType())) {
  ...
  } else if (auto scalarPtrType = ptrType.dyn_cast<triton_rocm::PointerType>()) {
