...
bool isZero(mlir::Value val) {
  ...
  if (auto bc = val.getDefiningOp<mlir::triton_rocm::BroadcastOp>()) {
...
public:
  CombineSelectMaskedLoadPattern(mlir::MLIRContext *context)
      : mlir::RewritePattern(mlir::arith::SelectOp::getOperationName(), 3,
                             context, {triton_rocm::LoadOp::getOperationName()}) {}
  ...
  matchAndRewrite(mlir::Operation *op,
    ...
    auto loadOp = llvm::dyn_cast_or_null<triton_rocm::LoadOp>(loadOpCandidate);
    if (!loadOp)
      return mlir::failure();
    ...
    auto broadcastOp =
        llvm::dyn_cast_or_null<triton_rocm::BroadcastOp>(broadcastOpCandidate);
    if (!broadcastOp)
      return mlir::failure();
...
public:
  CombineBroadcastMulReducePattern(mlir::MLIRContext *context)
      : mlir::RewritePattern(triton_rocm::ReduceOp::getOperationName(), 1, context) {
  }

  mlir::LogicalResult matchAndRewrite(mlir::Operation *op,
                                      mlir::PatternRewriter &rewriter) const {
    auto reduceOp = llvm::dyn_cast<triton_rocm::ReduceOp>(op);
    if (!reduceOp)
      return mlir::failure();
    ...
    auto broadcastLhsOp = llvm::dyn_cast_or_null<triton_rocm::BroadcastOp>(
        mulOp.getOperand(0).getDefiningOp());
    if (!broadcastLhsOp)
      return mlir::failure();
    auto broadcastRhsOp = llvm::dyn_cast_or_null<triton_rocm::BroadcastOp>(
        mulOp.getOperand(1).getDefiningOp());
    if (!broadcastRhsOp)
      return mlir::failure();
    // broadcast operand is expand dims
    auto expandLhsOp = llvm::dyn_cast_or_null<triton_rocm::ExpandDimsOp>(
        broadcastLhsOp.getOperand().getDefiningOp());
    if (!expandLhsOp)
      return mlir::failure();
    auto expandRhsOp = llvm::dyn_cast_or_null<triton_rocm::ExpandDimsOp>(
    ...
    auto newAcc = rewriter.create<triton_rocm::SplatOp>(
        op->getLoc(), newAccType,
        rewriter.create<arith::ConstantOp>(op->getLoc(),
                                           rewriter.getF32FloatAttr(0)));
    rewriter.replaceOpWithNewOp<triton_rocm::DotOp>(op, expandLhsOp.getOperand(),
...
std::unique_ptr<mlir::Pass> mlir::triton_rocm::createCombineOpsPass() {
