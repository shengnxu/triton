...
bool isSplat(Operation *op) {
  if (auto splatOp = llvm::dyn_cast<triton_rocm::SplatOp>(op)) {
    return true;
  }
...
struct MoveSplatAfterElementwisePattern
  ...
  void rewrite(Operation *op, mlir::PatternRewriter &rewriter) const override {
    ...
    for (unsigned iOp = 0; iOp < operands.size(); ++iOp) {
      ...
      DenseElementsAttr constAttr;
      if (auto splatOp = llvm::dyn_cast<triton_rocm::SplatOp>(definingOp)) {
    ...
    for (unsigned iRes = 0; iRes < resultTypes.size(); ++iRes) {
      auto newResult = rewriter.create<triton_rocm::SplatOp>(loc, resultTypes[iRes],
...
struct MoveBroadcastAfterElementwisePattern
  ...
  mlir::LogicalResult match(Operation *op) const override {
    ...
    for (auto operand : operands) {
      ...
      auto getSrcShape = [](triton_rocm::BroadcastOp b) {
        return b.getSrc().getType().cast<RankedTensorType>().getShape();
      };
      if (auto broadcastOp = llvm::dyn_cast<triton_rocm::BroadcastOp>(definingOp)) {
        if (!seenBroadcast) {
          seenBroadcast = true;
  ...
  void rewrite(Operation *op, mlir::PatternRewriter &rewriter) const override {
    ...
    // Find broadcast op
    auto operands = op->getOperands();
    triton_rocm::BroadcastOp broadcastOp;
    for (auto operand : operands) {
      broadcastOp = operand.getDefiningOp<triton_rocm::BroadcastOp>();
      if (broadcastOp) {
        break;
      }
    }
    ...
    for (auto operand : operands) {
      ...
      if (auto broadcastSrcOp =
              llvm::dyn_cast<triton_rocm::BroadcastOp>(definingOp)) {
      ...
      if (auto splatOp = llvm::dyn_cast<triton_rocm::SplatOp>(definingOp)) {
        auto newSplat =
            rewriter.create<triton_rocm::SplatOp>(loc, newTy, splatOp.getSrc());
    ...
    for (unsigned iRes = 0; iRes < newResultTypes.size(); ++iRes) {
      auto newResult = rewriter.create<triton_rocm::BroadcastOp>(
...
public:
  void runOnOperation() override {
    ...
    patterns.add<CanonicalizePattern<triton_rocm::BroadcastOp>>(context);
    patterns.add<CanonicalizePattern<triton_rocm::ExpandDimsOp>>(context);
    // elementwise(broadcast(a)) => broadcast(elementwise(a))
...
std::unique_ptr<mlir::Pass> mlir::triton_rocm::createReorderBroadcastPass() {
