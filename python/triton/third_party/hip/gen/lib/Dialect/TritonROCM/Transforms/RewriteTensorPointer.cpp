...
public:
  ...
  Value getExpandedOffsetWithRange(OpBuilder &builder, const Location &loc,
    ...
    Value splatOffset =
        builder.create<triton_rocm::SplatOp>(loc, indexRowType, offsets[i]);
    Value range = builder.create<triton_rocm::MakeRangeOp>(loc, indexI32RowType, 0,
                                                      tensorShape[i]);
    ...
    for (int j = 0; j < tensorShape.size(); ++j) {
      if (j == i)
        continue;
      expandedResult =
          builder.create<triton_rocm::ExpandDimsOp>(loc, expandedResult, j);
    }
  ...
  Value generatePtr(OpBuilder &builder, const Location &loc) {
    ...
    auto ptrType = base.getType().cast<triton_rocm::PointerType>();
    auto ptrTensorType = RankedTensorType::get(tensorShape, ptrType);

    // Generate offsets per dimension
    Value ptr = builder.create<triton_rocm::SplatOp>(loc, ptrTensorType, base);
    for (unsigned i = 0; i < tensorShape.size(); ++i) {
      ...
      Value splatStride = builder.create<triton_rocm::SplatOp>(
          loc, offsetWithRange.getType(), strides[i]);
      Value offsetWithStride =
          builder.create<arith::MulIOp>(loc, offsetWithRange, splatStride);
      Value broadcasted = builder.create<triton_rocm::BroadcastOp>(
          loc, indexTensorType, offsetWithStride);

      // Add to the pointer
      ptr = builder.create<triton_rocm::AddPtrOp>(loc, ptrTensorType, ptr,
                                             broadcasted);
    }
  ...
  Value generateMask(OpBuilder &builder, const Location &loc,
    ...
    for (auto i : boundaryCheck.value()) {
      ...
      Value splatLowerBound = builder.create<triton_rocm::SplatOp>(
      ...
      Value splatUpperBound = builder.create<triton_rocm::SplatOp>(
      ...
      Value broadcasted =
          builder.create<triton_rocm::BroadcastOp>(loc, maskTensorType, andResult);
  ...
  Value generateOther(OpBuilder &builder, const Location &loc,
                      const std::optional<triton_rocm::PaddingOption> &padding) {
    if (!padding.has_value())
      return Value();

    // Create element attribute
    auto elementType =
        base.getType().cast<triton_rocm::PointerType>().getPointeeType();
    ...
    if (padding.value() == triton_rocm::PaddingOption::PAD_NAN) {
    ...
    return builder.create<triton_rocm::SplatOp>(loc, otherTensorType, constant);
  }
};
...
public:
  ...
  static bool needRewrite(Operation *op) {
    return std::any_of(op->getOperands().begin(), op->getOperands().end(),
                       [](Value operand) {
                         return triton_rocm::isTensorPointerType(operand.getType());
                       });
  }
  ...
  Operation *rewriteMakeTensorPtrOp(OpBuilder &builder,
                                    triton_rocm::MakeTensorPtrOp op,
                                    std::stack<Operation *> &eraser) {
    // Save info for later use
    auto ptrType = op.getResult().getType().cast<triton_rocm::PointerType>();
  ...
  Operation *rewriteAdvanceOp(OpBuilder &builder, triton_rocm::AdvanceOp op,
                              std::stack<Operation *> &eraser) {
  ...
  Operation *rewriteLoadStoreOp(OpBuilder &builder, Operation *op,
                                std::stack<Operation *> &eraser) {
    assert(isa<triton_rocm::LoadOp>(op) || isa<triton_rocm::StoreOp>(op));

    // We only have to rewrite load/stores with tensor pointers
    auto ptr = op->getOperand(0);
    if (!triton_rocm::isTensorPointerType(ptr.getType()))
      return nullptr;
    ...
    if (auto loadOp = dyn_cast<triton_rocm::LoadOp>(op)) {
      assert(!loadOp.getMask() && !loadOp.getOther());
      boundaryCheck = loadOp.getBoundaryCheck();
    } else if (auto storeOp = dyn_cast<triton_rocm::StoreOp>(op)) {
    ...
    if (auto loadOp = dyn_cast<triton_rocm::LoadOp>(op)) {
      auto newResult = builder.create<triton_rocm::LoadOp>(
          loadOp.getLoc(), newPtr, newMask, newOther, loadOp.getCache(),
          loadOp.getEvict(), loadOp.getIsVolatile());
      op->getResult(0).replaceAllUsesWith(newResult);
    } else if (auto storeOp = dyn_cast<triton_rocm::StoreOp>(op)) {
      builder.create<triton_rocm::StoreOp>(storeOp.getLoc(), newPtr,
  ...
  Operation *rewriteForOp(OpBuilder &builder, scf::ForOp op,
    ...
    for (unsigned i = 0, oldI = 0, size = op.getInitArgs().size(); i < size;
         ++i, ++oldI) {
      if (!triton_rocm::isTensorPointerType(newIterOperands[i].getType()))
        continue;
    ...
    for (unsigned i = 0, oldI = 0, sz = op.getInitArgs().size(); oldI < sz;
      ...
      if (triton_rocm::isTensorPointerType(oldRegionIterArg.getType())) {
    ...
    for (unsigned i = 0, oldI = 0; oldI < op.getNumResults(); ++i, ++oldI) {
      ...
      if (triton_rocm::isTensorPointerType(oldResult.getType())) {
  ...
  Operation *rewriteYieldOp(OpBuilder &builder, scf::YieldOp op,
    ...
    for (unsigned i = 0, size = op.getNumOperands(); i < size; ++i) {
      if (!triton_rocm::isTensorPointerType(newOperands[i].getType()))
        continue;
  ...
  Operation *rewriteOp(Operation *op, std::stack<Operation *> &eraser) {
    ...
    } else if (auto advanceOp = dyn_cast<triton_rocm::AdvanceOp>(op)) {
      return rewriteAdvanceOp(builder, advanceOp, eraser);
    } else if (isa<triton_rocm::LoadOp>(op) || isa<triton_rocm::StoreOp>(op)) {
...
std::unique_ptr<Pass>
triton_rocm::createRewriteTensorPointerPass(int computeCapability, bool isROCM) {
