...
#include "triton/Target/LLVMROCMIR/Passes.h"
...
static std::map<std::string, std::string> getExternLibs(mlir::ModuleOp module) {
  ...
  if (!funcs.empty()) {
    ...
    std::string env_path = ::triton_rocm::tools::getenv("TRITON_LIBDEVICE_PATH");
    if (!env_path.empty()) {
...
translateLLVMToLLVMIR(llvm::LLVMContext *llvmContext, mlir::ModuleOp module,
  ...
  const int numWarps = triton_rocm::gpu_rocm::TritonGPUROCMDialect::getNumWarps(module);
  const int warpSize = triton_rocm::gpu_rocm::TritonGPUROCMDialect::getThreadsPerWarp(module);
...
std::unique_ptr<llvm::Module>
translateTritonGPUROCMToLLVMIR(llvm::LLVMContext *llvmContext,
  ...
  pm.enableIRPrinting(
      ...
      [](mlir::Pass *pass, mlir::Operation *) {
        return ::triton_rocm::tools::getBoolEnv("MLIR_ENABLE_DUMP");
      },
  ...
  pm.addPass(
      createConvertTritonGPUROCMToLLVMPass(computeCapability, target, &tmaInfos));
#if 0
  pm.addPass(createConvertNVGPUROCMToLLVMPass());
#endif
...
#endif
  if (!::triton_rocm::tools::getBoolEnv("TRITON_DISABLE_LINE_INFO"))
  ...
  if (::triton_rocm::tools::getBoolEnv("LLVM_IR_ENABLE_DUMP")) {
