...
from ......._C.librocm_backend_for_triton.triton_rocm import (ClusterInfo, TMAInfos, add_external_libs, compile_ptx_to_cubin, get_env_vars,
...
from ..runtime.jit import (JITFunctionROCM, get_cuda_stream, get_current_device, get_device_capability)
from ..tools.disasm import get_sass
from .code_generator import ast_to_ttir_rocm
...
def optimize_ttir_rocm(mod, target):
...
def ttir_to_ttgir_rocm(mod, num_warps, warpsize, num_ctas, target):
    pm = ir.pass_manager(mod.context)
    pm.enable_debug()
    if is_hip():
...
def optimize_ttgir_rocm(mod, num_stages, num_warps, num_ctas, target, cluster_info, enable_warp_specialization,
...
def ttgir_to_llir_rocm(mod, extern_libs, target, tma_infos, waves_per_eu=0):
    if extern_libs:
...
def make_hash(fn, target, env_vars, device_backend, **kwargs):
    ...
    if isinstance(fn, JITFunctionROCM):
...
def parse_mlir_module_rocm(path, context):
    module = ir.parse_mlir_module_rocm(path, context)
...
def compile(fn, **kwargs):
    ...
    stages["ttir"] = (lambda path: parse_mlir_module_rocm(path, context), lambda src: optimize_ttir_rocm(
        ast_to_ttir_rocm(src, signature, configs[0], constants, debug=debug, target=target), target))
    if is_cuda:
        stages["ttgir"] = (lambda path: parse_mlir_module_rocm(path, context), lambda src: optimize_ttgir_rocm(
            ttir_to_ttgir_rocm(src, num_warps, num_ctas, target), num_stages, num_warps, num_ctas, target, cluster_info,
            enable_warp_specialization, enable_persistent, optimize_epilogue))
        stages["llir"] = (lambda path: Path(path).read_text(),
                          lambda src: ttgir_to_llir_rocm(src, extern_libs, target, tma_infos))
        ...
        stages["ttgir"] = (lambda path: parse_mlir_module_rocm(path, context),
                           lambda src: optimize_ttgir_rocm(ttir_to_ttgir_rocm(src, num_warps, num_ctas, arch), num_stages, num_warps, num_ctas, arch, cluster_info, enable_warp_specialization, enable_persistent, optimize_epilogue))
        stages["llir"] = (lambda path: Path(path).read_text(),
                          lambda src: ttgir_to_llir_rocm(src, extern_libs, arch, tma_infos))
    ...
    ids_of_folded_args = tuple([int(k) for k in configs[0].ids_of_folded_args]) if isinstance(fn, JITFunctionROCM) else ()
    ...
    if isinstance(fn, JITFunctionROCM) and "tensormaps_info" in metadata:
        fn.tensormaps_info = metadata["tensormaps_info"]

    ids_of_const_exprs = tuple(fn.constexprs) if isinstance(fn, JITFunctionROCM) else ()
    ids = {
