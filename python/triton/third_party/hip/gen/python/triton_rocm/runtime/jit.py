...
from ......._C.librocm_backend_for_triton.triton_rocm import TMAInfos
...
class DependenciesFinder(ast.NodeVisitor):
    """
    This AST visitor is used to find dependencies of a JITFunctionROCM. This can
    be used to invalidate a JITFunctionROCM's hash when its source code -- or
...
class KernelArg:
    ...
    def signature_key(self):
        ...
        if "Tensor" in annotation:
            ...
            return JITFunctionROCM._key_of(self.value)

    def specialization_key(self):
        assert not self.param.do_not_specialize

        try:
            return (self.value.data_ptr() % JITFunctionROCM.divisibility == 0, )
        ...
        if isinstance(self.value, int):
            ...
            return (
                self.value % JITFunctionROCM.divisibility == 0,
                self.value % JITFunctionROCM.divisibility_8 == 0,
                self.value == 1,
            )
...
class KernelInterface(Generic[T]):
    ...
    def __getitem__(self, grid) -> T:
        ...
        Hence JITFunctionROCM.__getitem__ returns a callable proxy that
        memorizes the grid.
        """
        return cast(T, functools.partial(cast(Callable, self.run), grid=grid))


class JITFunctionROCM(KernelInterface[T]):
    ...
    def _spec_of(arg):
        if hasattr(arg, "data_ptr"):
            return arg.data_ptr() % JITFunctionROCM.divisibility == 0
        elif isinstance(arg, int):
    ...
    def _get_config(self, *args):
        ...
        def is_divisible_by_16(x):
            if hasattr(x, "data_ptr"):
                return x.data_ptr() % JITFunctionROCM.divisibility == 0
            elif isinstance(x, int):
                return x % JITFunctionROCM.divisibility == 0
            if x is None:
                return True
            return False
        ...
        def is_divisible_by_8(x):
            if isinstance(x, int):
                return x % JITFunctionROCM.divisibility_8 == 0
            if x is None:
                return True
            return False
    ...
    def _call_hook(
        ...
        if JITFunctionROCM.cache_hook is None:
            return False
        ...
        return JITFunctionROCM.cache_hook(
            key=key,
            repr=repr,
    ...
    def __setattr__(self, name, value):
        super(JITFunctionROCM, self).__setattr__(name, value)
    ...
    def __repr__(self):
        return f"JITFunctionROCM({self.module}:{self.fn.__name__})"
...
@overload
def jit(fn: T) -> JITFunctionROCM[T]:
    ...
...
def jit(
    ...
    *,
    ...
    debug: Optional[bool] = None,
    noinline: Optional[bool] = None,
) -> Union[JITFunctionROCM[T], Callable[[T], JITFunctionROCM[T]]]:
    """
    ...
    def decorator(fn: T) -> JITFunctionROCM[T]:
        assert callable(fn)
        if os.getenv("TRITON_INTERPRET", "0") == "1":
            return InterpretedFunction(fn)
        else:
            return JITFunctionROCM(
                fn,
                version=version,
