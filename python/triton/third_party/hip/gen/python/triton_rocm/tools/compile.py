...
import triton.third_party.hip.gen.python.triton_rocm
from triton.third_party.hip.gen.python.triton_rocm.compiler.code_generator import kernel_suffix
from triton.third_party.hip.gen.python.triton_rocm.compiler.make_launcher import ty_to_cpp
...
will compile triton.JITFunctionROCM of name `kernel` inside the file `/path/to/kernel.py`.
...
if __name__ == "__main__":
    ...
    config = triton.third_party.hip.gen.python.triton_rocm.compiler.instance_descriptor(divisible_by_16=divisible_by_16, equal_to_1=equal_to_1)
    for i in equal_to_1:
        constexprs.update({i: 1})
    ccinfo = triton.third_party.hip.gen.python.triton_rocm.compile(kernel, signature=signature, constants=constexprs, configs=[config],
