import hashlib
from logging import debug, info, log, warn
import os
import re
import subprocess
import tempfile
...
from triton.common.backend import (
    BaseBackend,
    register_backend,
...
if HIP_BACKEND_MODE:
    from ..._C.librocm_backend_for_triton import triton_rocm as _triton
    from .gen.python.triton_rocm.compiler.compiler import (
        optimize_ttgir_rocm,
    ...
    from .gen.python.triton_rocm.compiler.code_generator import ast_to_ttir_rocm
    from .gen.python.triton_rocm.runtime import JITFunctionROCM
else:
...
def make_stub(name, signature, constants, ids, **kwargs):
    ...
    so_cache_key = make_so_cache_key(
        compute_core_version_key(), signature, constants, ids, **kwargs
    )
...
def get_arch_info():
    # e.g. amdgcn-amd-amdhsa--gfx908:sramecc+:xnack-
    ARCH_INFO: str = _triton.get_arch_info()
    if ARCH_INFO:
        return ARCH_INFO
    else:
        # try rocminfo on the machine
        rocm_path_dir = os.getenv("ROCM_PATH", default="/opt/rocm")
        arch_info_from_rocminfo = subprocess.check_output(
            rocm_path_dir + "/bin/rocminfo"
        ).decode()
        if arch_info_from_rocminfo:
            return arch_info_from_rocminfo
    print("arch info not found!")
    return None
...
def get_amdgpu_arch_fulldetails():
    ...
    if gfx_arch_details:
        ...
        return {
            "gfx_triple": arch_triple,
            "gfx_arch": gfx_arch,
            "gfx_features": arch_features,
...
# GCN & HSACO translation
def llir_to_amdgcn_and_hsaco(
    mod: Any, gfx_arch: str, gfx_triple: str, gfx_features: str
) -> Tuple[str, str]:
    """
    ...
    """
    if HIP_BACKEND_MODE:
        return _triton.translate_llvmir_to_hsaco(
...
class HIPBackend(BaseBackend):
        ...
        if self.is_standalone():
            ...
            stages["ttgir"] = (
                ...
                lambda src: optimize_ttgir(
                    ttir_to_ttgir(src, num_warps, warp_size, num_ctas, arch),
                    num_stages,
                    num_warps,
                    num_ctas,
                    arch,
            ...
            stages["llir"] = (
                ...
                lambda src: ttgir_to_llir(
                    src, extern_libs, arch, tma_infos, waves_per_eu
                ),
            )
            ...
            stages["amdgcn"] = (
                ...
                lambda src: llir_to_amdgcn_and_hsaco(
                    src, gfx_arch, gfx_triple, gfx_features
                ),
            )
        else:
            num_warps = arch["num_warps"]
            ...
            # get new context we will reregister the dialects we need
            context = _triton.ir.context()  # other["context"]
            warp_size = other["warp_size"]
            # convert cluster_info to our forks type
            cluster_info = _triton.ClusterInfo()
            cluster_info.clusterDimX = other["cluster_info"].clusterDimX
            cluster_info.clusterDimY = other["cluster_info"].clusterDimY
            cluster_info.clusterDimZ = other["cluster_info"].clusterDimZ
            enable_warp_specialization = other["enable_warp_specialization"]
            enable_persistent = other["enable_persistent"]
            optimize_epilogue = other["optimize_epilogue"]
            tma_infos = other["tma_infos"]
            ...
            def ttir_to_ttir_rocm(mod):
                with tempfile.NamedTemporaryFile(
                    mode="w", delete=False, suffix=".ttir"
                ) as temp_file:
                    temp_file.write(str(mod))
                    temp_file_path = temp_file.name
                return parse_mlir_module_rocm(temp_file_path, context)

            stages["ttgir"] = (
                lambda path: parse_mlir_module_rocm(path, context),
                lambda src: optimize_ttgir_rocm(
                    ttir_to_ttgir_rocm(
                        ttir_to_ttir_rocm(src), num_warps, warp_size, num_ctas, arch
                    ),
                    num_stages,
                    num_warps,
                    num_ctas,
    ...
    def get_architecture_descriptor(self, **kwargs):
        ...
        arch["num_ctas"] = 1
        warp_size_queried = (
            _triton.get_warp_size()
        )  # TODO: query the warp size. Not working for in CI
        arch["warp_size"] = warp_size_queried if warp_size_queried else 64
        return arch
