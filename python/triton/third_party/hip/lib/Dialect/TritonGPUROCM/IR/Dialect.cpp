...
bool isSharedEncoding(Value value) {
  ...
  if (auto tensorType = type.dyn_cast<RankedTensorType>()) {
    ...
    return encoding && encoding.isa<triton::gpu_rocm::SharedEncodingAttr>();
  }
  return false;
}
...
bool isExpensiveCat(CatOp cat, Attribute targetEncoding) {
  ...
  auto totalElemsPerThread = gpu_rocm::getTotalElemsPerThread(tensorTy);
  auto shape = tensorTy.getShape();
  auto elemTy = tensorTy.getElementType();
  auto newTotalElemsPerThread =
      gpu_rocm::getTotalElemsPerThread(targetEncoding, shape, elemTy);
...
struct TritonGPUInferLayoutInterface
  ...
  verifyDotOpEncodingCompatibility(Operation *op, Attribute operandEncodingA,
    ...
    auto aEncoding =
        operandEncodingA.dyn_cast<triton::gpu_rocm::DotOperandEncodingAttr>();
    auto bEncoding =
        operandEncodingB.dyn_cast<triton::gpu_rocm::DotOperandEncodingAttr>();
...
LogicalResult ConvertLayoutOp::canonicalize(ConvertLayoutOp op,
  ...
  if (dstType.getEncoding().isa<triton::gpu_rocm::DotOperandEncodingAttr>() &&
      (srcType.getEncoding().isa<triton::gpu_rocm::MmaEncodingAttr>() ||
       srcType.getEncoding().isa<triton::gpu_rocm::MfmaEncodingAttr>()))
    return mlir::failure();
  // for hopper MMAv3
  if (!op.use_empty()) {
    ...
    if (hasDotUser) {
      if (dstType.getEncoding().isa<triton::gpu_rocm::SharedEncodingAttr>() &&
          srcType.getEncoding().isa<triton::gpu_rocm::MmaEncodingAttr>())
        return mlir::failure();
    }
  }
  ...
  auto alloc_tensor = dyn_cast<triton::gpu_rocm::AllocTensorOp>(arg);
  if (alloc_tensor) {
    if (!triton::gpu_rocm::isSharedEncoding(op->getResult(0))) {
      return mlir::failure();
    }
    rewriter.replaceOpWithNewOp<triton::gpu_rocm::AllocTensorOp>(
        op, op->getResult(0).getType());
    return mlir::success();
  }
  // cvt(insert_slice(x), type2) -> insert_slice(cvt(x, type2))
  auto insert_slice = dyn_cast<triton::gpu_rocm::InsertSliceAsyncOp>(arg);
  if (insert_slice) {
    if (!triton::gpu_rocm::isSharedEncoding(op->getResult(0))) {
      return mlir::failure();
    }
    ...
    auto newArg = rewriter.create<triton::gpu_rocm::ConvertLayoutOp>(
        op->getLoc(), newType, insert_slice.getDst());
    rewriter.replaceOpWithNewOp<triton::gpu_rocm::InsertSliceAsyncOp>(
  ...
  auto extract_slice = dyn_cast<triton::gpu_rocm::ExtractSliceOp>(arg);
  if (extract_slice) {
    if (!triton::gpu_rocm::isSharedEncoding(op->getResult(0))) {
      return mlir::failure();
    }
    ...
    rewriter.replaceOpWithNewOp<triton::gpu_rocm::ExtractSliceOp>(
  ...
  if (llvm::isa<triton::gpu_rocm::ConvertLayoutOp>(arg)) {
    if (arg->getOperand(0).getDefiningOp() &&
        !triton::gpu_rocm::isSharedEncoding(arg->getOperand(0)) &&
        triton::gpu_rocm::isSharedEncoding(op.getOperand()) &&
        !triton::gpu_rocm::isSharedEncoding(op.getResult())) {
      return mlir::failure();
    }
    if (triton::gpu_rocm::isSharedEncoding(op.getOperand()) &&
        triton::gpu_rocm::isSharedEncoding(op.getResult())) {
      return mlir::failure();
    }
    auto srcType = op.getOperand().getType().cast<RankedTensorType>();
    auto srcShared =
        srcType.getEncoding().dyn_cast<triton::gpu_rocm::SharedEncodingAttr>();
    if (srcShared && srcShared.getVec() > 1)
      return mlir::failure();
    rewriter.replaceOpWithNewOp<triton::gpu_rocm::ConvertLayoutOp>(
