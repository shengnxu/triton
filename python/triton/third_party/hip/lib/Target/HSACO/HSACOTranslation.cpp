...
void translateTritonToTritonGPUROCM(mlir::ModuleOp &module,
                                    ...
                                    int numStages) {
  // std::cout << "translateTritonToTritonGPUROCM" << std::endl;

  // triton to triton gpu
  mlir::PassManager pm(module.getContext());
  pm.addPass(
      mlir::triton::createConvertTritonToTritonGPUROCMPass(numWarps));

  // create file for IRs
  std::error_code EC;
  llvm::raw_fd_ostream fileOS("log/translateTritonToTritonGPUROCM.log", EC,
                              llvm::sys::fs::OF_Text);
  ...
  pm.addPass(mlir::createTritonGPUROCMCoalescePass());
  pm.addPass(mlir::createTritonGPUROCMRemoveLayoutConversionsPass());
  pm.addPass(mlir::createTritonGPUROCMAccelerateMatmulPass(computeCapability));
  pm.addPass(mlir::createTritonGPUROCMRemoveLayoutConversionsPass());
  pm.addPass(mlir::createTritonGPUROCMOptimizeDotOperandsPass());
  pm.addPass(mlir::createTritonGPUROCMPipelinePass(numStages));
  pm.addPass(mlir::createTritonGPUROCMPrefetchPass());
  pm.addPass(mlir::createTritonGPUROCMOptimizeDotOperandsPass());
  pm.addPass(mlir::createTritonGPUROCMRemoveLayoutConversionsPass());
  pm.addPass(mlir::createTritonGPUROCMDecomposeConversionsPass());
  pm.addPass(mlir::createTritonGPUROCMReorderInstructionsPass());
...
static std::map<std::string, std::string> getExternLibs(mlir::ModuleOp module) {
  ...
  if (module.getOperation()->hasAttr("triton_gpu_rocm.externs")) {
    auto dict = module.getOperation()
                    ->getAttr("triton_gpu_rocm.externs")
  ...
  if (!funcs.empty()) {
    ...
    if (fs::exists(runtime_path)) {
      ...
      if (!fs::exists(compiletime_path)) {
        std::string error_msg = "[third_party HSACOTranslation] Can't find libdevice at neither " +
...
translateLLVMDialectToLLVMIR(llvm::LLVMContext *llvmContext,
  ...
  const int numWarps = mlir::triton::gpu_rocm::TritonGPUROCMDialect::getNumWarps(module);
  const int warpSize = mlir::triton::gpu_rocm::TritonGPUROCMDialect::getThreadsPerWarp(module);
...
void translateTritonGPUROCMToLLVMDialect(mlir::ModuleOp &module,
  ...
  // std::cout << "translateTritonGPUROCMToLLVMDialect" << std::endl;
  mlir::PassManager pm(module.getContext());
  ...
  // create file for IRs
  std::error_code EC;
  llvm::raw_fd_ostream fileOS("log/translateTritonGPUROCMToLLVMDialect.log", EC,
                              llvm::sys::fs::OF_Text);
  ...
  pm.addPass(mlir::triton::createConvertTritonGPUROCMToLLVMPass());
...
void addExternalLibs(mlir::ModuleOp &module,
  ...
  // std::cout << "addExternalLibs" << std::endl;
  ...
  module.getOperation()->setAttr("triton_gpu_rocm.externs", dict);
}
...
translateTritonIRToHSACO(mlir::ModuleOp module, std::string gfx_arch,
  ...
  if (!llvmModule) {
    llvm::errs() << "Translate to LLVM IR failed"
                 << "\n";
    llvm::report_fatal_error("Failed to translate TritonGPUROCM to LLVM IR.");
  }
