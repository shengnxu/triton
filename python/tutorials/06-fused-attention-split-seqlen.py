...
@triton.jit
def _attn_fwd_inner(
    acc_1, acc_2, l_i_1, l_i_2, m_i_1, m_i_2, q1, q2,
...
def _attn_fwd(
    ...
    Q_block_ptr = tl.make_block_ptr(
        ...
        block_shape=(BLOCK_K, BLOCK_DMODEL),
        order=(1, 0),
    )
    ...
    offs_k_1 = start_m * BLOCK_M + tl.arange(0, BLOCK_K)
    offs_k_2 = start_m * BLOCK_M + tl.arange(BLOCK_K, 2*BLOCK_K)
    # initialize pointer to m and l
    m_i_1 = tl.zeros([BLOCK_K], dtype=tl.float32) - float("inf")
    m_i_2 = tl.zeros([BLOCK_K], dtype=tl.float32) - float("inf")
    l_i_1 = tl.zeros([BLOCK_K], dtype=tl.float32) + 1.0
    l_i_2 = tl.zeros([BLOCK_K], dtype=tl.float32) + 1.0
    
    acc_1 = tl.zeros([BLOCK_K, BLOCK_DMODEL], dtype=tl.float32)
    acc_2 = tl.zeros([BLOCK_K, BLOCK_DMODEL], dtype=tl.float32)
    ...
    Q_block_ptr = tl.advance(Q_block_ptr, (BLOCK_K, 0))
    q2 = tl.load(Q_block_ptr)

    q1 = (q1 * qk_scale).to(q1.dtype)
    q2 = (q2 * qk_scale).to(q2.dtype)
    # stage 1: off-band
    # For causal = True, STAGE = 3 and _attn_fwd_inner gets 1 as its STAGE
    # For causal = False, STAGE = 1, and _attn_fwd_inner gets 3 as its STAGE
    if STAGE & 1:
        acc_1, acc_2, l_i_1, l_i_2, m_i_1, m_i_2 = _attn_fwd_inner(
            acc_1, acc_2, l_i_1, l_i_2, m_i_1, m_i_2, q1, q2, K_block_ptr, V_block_ptr,
            start_m,
    ...
    #tl.store(m_ptrs, m_i_1 + tl.math.log2(l_i_1))
    tl.store(O_block_ptr, acc_1.to(tl.float16))
    O_block_ptr = tl.advance(O_block_ptr, (BLOCK_K, 0))
    tl.store(O_block_ptr, acc_2.to(tl.float16))
...
class _attention(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, causal, sm_scale, split_kernel=False):
        ...
        _attn_fwd[grid](
            ...
            N_CTX=q.shape[2],
            BLOCK_DMODEL=Lk,
            STAGE=stage, matrix_instr_nonkdim=16
        )
