...
def test_op_fwd(Z, H, N_CTX, D_HEAD, P_SEQ, causal, dtype=torch.float16):
    ...
    if TORCH_HAS_FP8:
        q = q.to(triton_to_torch_types[input_dtype])
        k = k.to(triton_to_torch_types[input_dtype])
        v = v.to(triton_to_torch_types[input_dtype])
    sm_scale = q.shape[-1] ** (-0.5)
    dout = torch.randn_like(q)
    # reference implementation
    M = torch.tril(torch.ones((N_CTX, N_CTX + P_SEQ), device="cuda"), diagonal=P_SEQ)
    p = torch.matmul(q.half(), k.half().transpose(2, 3)) * sm_scale
    if causal:
        p[:, :, M == 0] = float("-inf")
