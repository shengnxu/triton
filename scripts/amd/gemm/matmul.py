...
def get_configs_io_bound(use_split_k):
    configs = []
...
#       provided configs
@triton.autotune(
    configs= get_configs_io_bound(True) if tuning_full_space else [
...
# Kernel no split K
@triton.autotune(
    configs= get_configs_io_bound(False) if tuning_full_space else [
...
def main():
    ...
    for (m, n, k) in mnks:
        min_ms, flops = run_speed(m, n, k, dtype, 'triton')
        if args.compare:
