...
while read line; do
  ...
  driver='test_mfma_gemm.py'

  # command to run matmul
  # run_cmd="python ${driver} \
  #         -m ${M} -n ${N} -k ${K} -blockM ${BLOCK_M} \
  #         -blockN ${BLOCK_N} -blockK ${BLOCK_K} -splitK ${SPLIT_K} \
  #         -mPerWave ${m_per_wave} -kpack ${k_pack} -num_warps ${num_warps}"
  run_cmd="python ${driver} -m ${M} -n ${N} -k ${K}"
  # if [[ ${#params[@]} == 8 ]]; then
  #   run_cmd="${run_cmd} -groupM ${num_groups}"
  # fi
  ...
  for ((i=0; i<$iter_num; i++)); do
    ...
    if [[ $? != 0 ]]; then
      ...
      t=$(sed -n '/_kernel_0d/p' ${PROF_RESULT_FILE} | awk -F ',' '{print $4}')
      echo ${t} >> ${out_file}
    fi
  done
done < $config_file
