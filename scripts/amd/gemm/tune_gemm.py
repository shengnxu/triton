...
def profile_batch_kernels(M, N, K, gpuid, gpus, jobs, verbose):
    ...
    while jobId < jobs:
        ...
        run_bash_command_wrapper(f"rocprofv2 --plugin file --plugin-version 1 --kernel-trace -o {jobId} python {get_filename_profile_driver(M, N, K, jobId)}", capture=(verbose < 2))
        jobId += ngpus
...
def tune_gemm_config(M, N, K, col_a, col_b, dtype_a, dtype_b, dtype_c, init_type, configs,
    ...
    if not skipWarmup:
        ...
        fname = generate_compile_driver(M, N, K, col_a, col_b, dtype_a, dtype_b, dtype_c, init_type, configs, rotating_buffer_size, bias_size)
    ...
    generate_profile_tasks(M, N, K, col_a, col_b, dtype_a, dtype_b, dtype_c, init_type, configs, jobs, iters, run_bench, rotating_buffer_size, bias_size, icache_flush)
...
def main():
    ...
    for (M, N, K, col_a, col_b, myConfig) in mnks:
        ...
        if args.compare:
            ...
            test_correctness(M, N, K, col_a, col_b, dtype_a, dtype_b, dtype_c, init_type, bestConfig, bias_vector, False)
        elif not run_bench:
            print("", flush=True)
