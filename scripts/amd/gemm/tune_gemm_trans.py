...
from matmul_kernel_trans import matmul_kernel
...
def gen_kernel_and_configStr_from_config(M, N, K, config):
    ...
    matmul_def_str = f"""
def matmul_{configStr}(a, b, c, M, N, K, am, ak, bn, bk, cm, cn, warmup=False):
    #M, K = a.shape
    #K, N = b.shape
...
def try_config_{configStr}(M, N, K, am, ak, bn, bk, cm, cn, dtype):
...
"""
    ...
    with open("matmul_kernel_trans.py") as file:
